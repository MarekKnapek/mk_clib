...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_hash_stream_md2_test_compute_from_str_lit(char const(&str_lit)[str_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_cpp_constexpr_array_t<mk_sl_cui_uint8_t, mk_lib_crypto_hash_block_md2_digest_len> ret mk_lang_constexpr_init;

	mk_lang_static_assert(str_lit_len >= 1);

	msg = mk_lib_cpp_constexpr_str_lit_to_u8s(str_lit);
	mk_lib_crypto_hash_stream_md2_init(&md2);
	mk_lib_crypto_hash_stream_md2_append_u8(&md2, msg.data(), msg.size());
	mk_lib_crypto_hash_stream_md2_finish(&md2, &digest);
	for(i = 0; i != mk_lib_crypto_hash_block_md2_digest_len; ++i){ ret[i] = digest.m_uint8s[i]; }
	return ret;
}
