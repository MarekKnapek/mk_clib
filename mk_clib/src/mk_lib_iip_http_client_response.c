...
mk_lang_nodiscard mk_lang_constexpr static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_client_response_prrw_method_string_max_len(mk_lang_types_void_t) mk_lang_noexcept
{
	mk_lang_types_sint_t method_str_max;
	mk_lang_types_sint_t n;
	mk_lang_types_sint_t i;
	mk_lib_iip_http_client_response_method_id_t method_id;
	mk_lib_iip_http_strings_id_t method_str_id;
	mk_lang_types_sint_t method_str_len;

	method_str_max = 0;
	n = mk_lib_iip_http_client_response_method_id_e_dummy_end;
	for(i = 0; i != n; ++i)
	{
		method_id = ((mk_lib_iip_http_client_response_method_id_t)(i));
		method_str_id = mk_lib_iip_http_client_response_prrw_method_id_to_string_id(method_id); mk_lang_assert(method_str_id >= 0); mk_lang_assert(method_str_id < mk_lib_iip_http_strings_id_e_dummy_end);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_client_response_prrw_find_crlf(mk_lib_iip_http_client_response_pt const http) mk_lang_noexcept
{
	mk_sl_cui_uint8_pct dat_buf;
	mk_lang_types_sint_t dat_sise;
	mk_lang_types_sint_t n;
...
#include "mk_lang_warning_clang_push_conditional_uninitialized.h"
	...
	method_str_max = mk_lib_iip_http_client_response_prrw_method_string_max_len();
	dat_sise = mk_lib_iip_http_buffer_rw_sise(&http->m_buffer);
	...
	*consumed = rem;
	...
	if(i == n)
	{
		...
		{
			*error_code = mk_lib_iip_http_client_response_parse_error_code_e_bad_method;
		}
	}
	else
	{
		if(mk_lib_iip_http_client_response_prrw_is_space(http, method_str_len))
		{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_client_response_prrw_on_incoming_data_version_work(mk_lib_iip_http_client_response_pt const http, mk_sl_cui_uint8_pct const data_buf, mk_lang_types_sint_t const data_len, mk_lib_iip_http_client_response_parse_error_code_pt const error_code, mk_lang_types_sint_pt const consumed) mk_lang_noexcept
{
	...
	*consumed = rem;
	...
	if(is)
	{
		...
		http->m_version = mk_lib_iip_http_client_response_version_id_e_1_1;
	}
	return 0;
}
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_http_client_response_rw_construct(mk_lib_iip_http_client_response_pt const http) mk_lang_noexcept
{
	...
	http->m_method = mk_lib_iip_http_client_response_method_id_e_dummy_end;
	err = mk_lib_iip_http_buffer_rw_construct(&http->m_uri); mk_lang_check_rereturn(err);
	http->m_version = mk_lib_iip_http_client_response_version_id_e_dummy_end;
