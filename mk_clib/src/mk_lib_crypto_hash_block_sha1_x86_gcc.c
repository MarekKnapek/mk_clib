...
#if mk_lang_gnuc_is_at_least(11, 1) && (mk_lang_arch == mk_lang_arch_x8632 || mk_lang_arch == mk_lang_arch_x8664)
...
mk_lang_constexpr_static_inline mk_sl_cui_uint32_t const s_mk_lib_crypto_hash_block_sha1_x86_gcc_init[5] =
{
...
mk_lang_jumbo void mk_lib_crypto_hash_block_sha1_x86_gcc_init(mk_lib_crypto_hash_block_sha1_x86_gcc_pt const sha1) mk_lang_noexcept
{
	...
	mk_lang_static_assert(sizeof(mk_lib_crypto_hash_block_sha1_x86_gcc_block_t) == 64);
	mk_lang_static_assert(sizeof(mk_lib_crypto_hash_block_sha1_x86_gcc_digest_t) >= 20);

	mk_lang_assert(sha1);

	sha1->m_state[0] = s_mk_lib_crypto_hash_block_sha1_x86_gcc_init[0];
	sha1->m_state[1] = s_mk_lib_crypto_hash_block_sha1_x86_gcc_init[1];
	sha1->m_state[2] = s_mk_lib_crypto_hash_block_sha1_x86_gcc_init[2];
	sha1->m_state[3] = s_mk_lib_crypto_hash_block_sha1_x86_gcc_init[3];
	sha1->m_state[4] = s_mk_lib_crypto_hash_block_sha1_x86_gcc_init[4];
	mk_sl_cui_uint64_set_zero(&sha1->m_len);
}

mk_lang_jumbo void __attribute__((__target__("sse2,ssse3,sse4.1,sha"))) mk_lib_crypto_hash_block_sha1_x86_gcc_append_blocks(mk_lib_crypto_hash_block_sha1_x86_gcc_pt const sha1, mk_lib_crypto_hash_block_sha1_x86_gcc_block_pct const pblocks, mk_lang_types_usize_t const nblocks) mk_lang_noexcept
{
	#define mk_lib_crypto_hash_block_sha1_x86_gcc_reverse_32 ((0x0 << (3 * 2)) | (0x1 << (2 * 2)) | (0x2 << (1 * 2)) | (0x3 << (0 * 2)))
	...
	mk_lang_assert(nblocks <= mk_lang_limits_usize_max / mk_lib_crypto_hash_block_sha1_x86_gcc_block_len);
	...
	if(nblocks != 0)
	{
		...
		abcdx = _mm_shuffle_epi32(abcdx, mk_lib_crypto_hash_block_sha1_x86_gcc_reverse_32);
		e = _mm_set_epi32(*((int const*)(&sha1->m_state[4])), 0, 0, 0);
		for(iblock = 0, ptr = &pblocks[0].m_uint8s[0]; iblock != nblocks; ++iblock, ptr += mk_lib_crypto_hash_block_sha1_x86_gcc_block_len)
		{
			old_abcd = abcdx;
			old_e = e;
...
mk_lang_jumbo void mk_lib_crypto_hash_block_sha1_x86_gcc_finish(mk_lib_crypto_hash_block_sha1_x86_gcc_pt const sha1, mk_lib_crypto_hash_block_sha1_x86_gcc_block_pt const block, int const idx, mk_lib_crypto_hash_block_sha1_x86_gcc_digest_pt const digest) mk_lang_noexcept
{
	...
	mk_lang_assert(idx >= 0 && idx < mk_lib_crypto_hash_block_sha1_x86_gcc_block_len);
	mk_lang_assert(digest);

	ui = mk_lib_crypto_hash_block_sha1_x86_gcc_block_len;
	...
	rest = mk_lib_crypto_hash_block_sha1_x86_gcc_block_len - idx - 1;
	if(rest >= 8)
	{
		for(i = 0; i != rest - 8; ++i)
		{
	...
	else
	{
		...
		mk_lib_crypto_hash_block_sha1_x86_gcc_append_blocks(sha1, block, 1);
		for(i = 0; i != mk_lib_crypto_hash_block_sha1_x86_gcc_block_len - 8; ++i)
		{
			mk_sl_cui_uint8_set_zero(&block->m_uint8s[i]);
		}
	}
	mk_sl_uint_64_to_8_be(&ta, &block->m_uint8s[mk_lib_crypto_hash_block_sha1_x86_gcc_block_len - 8]);
	mk_lib_crypto_hash_block_sha1_x86_gcc_append_blocks(sha1, block, 1);
	for(i = 0; i != 5; ++i)
	{
