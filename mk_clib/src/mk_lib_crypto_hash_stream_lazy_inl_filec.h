...
mk_lang_constexpr mk_lang_jumbo void mk_lib_crypto_hash_stream_lazy_inl_defd_init(mk_lib_crypto_hash_stream_lazy_inl_defd_pt const hash) mk_lang_noexcept
{
	...
	mk_lib_crypto_hash_stream_lazy_inl_defd_base_init(&hash->m_base);
	hash->m_idx = 0;
}
...
mk_lang_constexpr mk_lang_jumbo void mk_lib_crypto_hash_stream_lazy_inl_defd_append_u8(mk_lib_crypto_hash_stream_lazy_inl_defd_pt const hash, mk_sl_cui_uint8_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	ptr = data;
	rem = size;
	free = mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len - hash->m_idx;
	if(rem > ((mk_lang_types_usize_t)(free)))
	{
		if(hash->m_idx != 0)
		{
			for(i = 0; i != free; ++i){ hash->m_block.m_uint8s[hash->m_idx + i] = ptr[i]; }
			mk_lib_crypto_hash_stream_lazy_inl_defd_base_append_blocks(&hash->m_base, &hash->m_block, 1);
			ptr += free;
			rem -= free;
			hash->m_idx = 0;
		}
		...
		else
		{
			for(j = 0; j != blocks; ++j)
			{
				for(i = 0; i != mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len; ++i){ hash->m_block.m_uint8s[i] =  ptr[j * mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len + i]; }
				mk_lib_crypto_hash_stream_lazy_inl_defd_base_append_blocks(&hash->m_base, &hash->m_block, 1);
			}
		}
		ptr += blocks * mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len;
		rem -= blocks * mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len;
	}
	mk_lang_assert(rem >= 0 && rem <= mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len);
	mk_lang_assert(((int)(rem)) + hash->m_idx <= mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len);
...
mk_lang_constexpr mk_lang_jumbo void mk_lib_crypto_hash_stream_lazy_inl_defd_append(mk_lib_crypto_hash_stream_lazy_inl_defd_pt const hash, mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_t block mk_lang_constexpr_init;
	...
	{
		n = size / mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len;
		for(j = 0; j != n; ++j)
		{
			for(i = 0; i != mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len; ++i){ mk_sl_cui_uint8_from_bi_uchar(&block.m_uint8s[i], &data[j * mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len + i]); }
			mk_lib_crypto_hash_stream_lazy_inl_defd_append_u8(hash, &block.m_uint8s[0], mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len);
		}
		m = size - n * mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len;
		if(m != 0)
		{
			for(i = 0; i != m; ++i){ mk_sl_cui_uint8_from_bi_uchar(&block.m_uint8s[i], &data[n * mk_lib_crypto_hash_stream_lazy_inl_defd_base_block_len + i]); }
			mk_lib_crypto_hash_stream_lazy_inl_defd_append_u8(hash, &block.m_uint8s[0], m);
		}
	}
}
...
mk_lang_constexpr mk_lang_jumbo void mk_lib_crypto_hash_stream_lazy_inl_defd_finish(mk_lib_crypto_hash_stream_lazy_inl_defd_pt const hash, mk_lib_crypto_hash_stream_lazy_inl_defd_base_digest_pt const digest) mk_lang_noexcept
{
	...
	mk_lib_crypto_hash_stream_lazy_inl_defd_base_finish(&hash->m_base, &hash->m_block, hash->m_idx, digest);
}


#include "mk_lib_crypto_hash_stream_lazy_inl_defu.h"
...
#undef mk_lib_crypto_hash_stream_lazy_t_name
