#ifndef mk_include_guard_mk_lib_iip_base64_fuzz_c
#define mk_include_guard_mk_lib_iip_base64_fuzz_c
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_base64_fuzz(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	mk_lang_types_pchar_t output_pcs_buf[mk_lang_roundup_div(mk_lang_countof(input_u8s_buf), 3) * 4];
	...
	mk_lib_iip_base64_encoder_fn(&input_u8s_buf[0], input_u8s_len, &output_pcs_buf[0], mk_lang_countof(output_pcs_buf), &output_pcs_len);
	...
	mk_lib_iip_base64_decoder_no_check(&output_pcs_buf[0], output_pcs_len, &output_u8s_buf[0], mk_lang_countof(output_u8s_buf), &output_u8s_len);
	...
	mk_sl_cui_uint8_memset_fn(&output_u8s_buf[0], &fill_u8, mk_lang_countof(output_u8s_buf));
	mk_lib_iip_base64_decoder_do_check(&output_pcs_buf[0], output_pcs_len, &output_u8s_buf[0], mk_lang_countof(output_u8s_buf), &output_u8s_len, &success);
	mk_lang_test(output_u8s_len >= 0);
	mk_lang_test(output_u8s_len <= mk_lang_countof(output_u8s_buf));
	...
	mk_lib_iip_base64_decoder_do_check(&input_pcs_buf[0], input_u8s_len, &output_u8s_buf[0], mk_lang_countof(output_u8s_buf), &output_u8s_len, &success);
	if(success)
	{
		mk_lang_test(output_u8s_len >= 0);
		mk_lang_test(output_u8s_len <= mk_lang_countof(output_u8s_buf));
