...
mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_xof_stream_blake3_fuzz(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	mk_lib_crypto_xof_stream_blake3_t blake3_a;
	mk_lang_types_ushort_t len;
	mk_lib_crypto_xof_stream_blake3_t blake3_b;
	...
	d = data;
	s = size;
	mk_lib_crypto_xof_stream_blake3_init(&blake3_a);
	if(!(s >= 2)) return;
	...
	s -= 2;
	if(!(s >= len)) return;
	mk_lib_crypto_xof_stream_blake3_append(&blake3_a, ((mk_lang_types_uchar_pct)(d)), len);
	d += len;
	s -= len;
	mk_lib_crypto_xof_stream_blake3_finish(&blake3_a);
	blake3_b = blake3_a;
	if(!(s >= 2)) return;
	len = ((mk_lang_types_ushort_t)(((mk_lang_types_ushort_t)(((mk_lang_types_ushort_t)(d[0])) << (0 * 8))) | ((mk_lang_types_ushort_t)(((mk_lang_types_ushort_t)(d[1])) << (1 * 8)))));
	d += 2;
	...
	mk_lib_crypto_xof_stream_blake3_squeeze(&blake3_a, digest_len, &digest_a[0]);
	if(!(s >= 2)) return;
	len = ((mk_lang_types_ushort_t)(((mk_lang_types_ushort_t)(((mk_lang_types_ushort_t)(d[0])) << (0 * 8))) | ((mk_lang_types_ushort_t)(((mk_lang_types_ushort_t)(d[1])) << (1 * 8)))));
	d += 2;
	s -= 2;
	...
	for(ipiece = 0; ipiece != npieces; ++ipiece)
	{
		...
		mk_lib_crypto_xof_stream_blake3_squeeze(&blake3_b, len, &digest_b[cursor]);
		cursor += len;
	}
	for(i = 0; i != cursor; ++i)
	{
