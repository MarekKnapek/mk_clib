#ifndef mk_include_guard_mk_lib_iip_cp_remote_destination_fuzz_c
#define mk_include_guard_mk_lib_iip_cp_remote_destination_fuzz_c
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_remote_destination_fuzz(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	gud = mk_lang_true;
	err = mk_lib_iip_cp_remote_destination_rw_from_bytes(&remote_destination, &destination_buf[0], destination_len, &gud, &consumed); mk_lang_check_rereturn(err);
	if(gud)
	{
