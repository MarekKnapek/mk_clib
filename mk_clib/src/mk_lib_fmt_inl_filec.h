...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_fmt_inl_defd_vsnnprintf(mk_lib_fmt_inl_defd_char_pt const dst_buf, mk_lang_types_sint_t const dst_len, mk_lib_fmt_inl_defd_char_pct const fmt_buf, mk_lang_types_sint_t const fmt_len, va_list* const va) mk_lang_noexcept
{
	...
	for(i = 0; i != n; ++i)
	{
		...
		if(ch == mk_lib_fmt_inl_defd_char_c('%'))
		{
			...
			else if(ch == mk_lib_fmt_inl_defd_char_c('s')){ mk_lib_fmt_inl_defd_vsnnprintf_s(dst, rem, va, &gud, &consumed); if(!gud){ break; } mk_lang_assert(consumed >= 1 && consumed <= rem); dst += consumed; rem -= consumed; }
			else if(ch == mk_lib_fmt_inl_defd_char_c('t')){ mk_lib_fmt_inl_defd_vsnnprintf_t(dst, rem, va, &gud, &consumed); if(!gud){ break; } mk_lang_assert(consumed >= 1 && consumed <= rem); dst += consumed; rem -= consumed; }
			else if(ch == mk_lib_fmt_inl_defd_char_c('n')){ mk_lib_fmt_inl_defd_vsnnprintf_n(dst, rem, va, &gud, &consumed); if(!gud){ break; } mk_lang_assert(consumed >= 1 && consumed <= rem); dst += consumed; rem -= consumed; }
			...
			else if(ch == mk_lib_fmt_inl_defd_char_c('l'))
			{
				...
				++i; mk_lang_assert(i != n);
				ch = fmt_buf[i];
				if(ch == mk_lib_fmt_inl_defd_char_c('x'))     { mk_lib_fmt_inl_defd_vsnnprintf_llx(dst, rem, va, &gud, &consumed); if(!gud){ break; } mk_lang_assert(consumed >= 1 && consumed <= rem); dst += consumed; rem -= consumed; }
				else if(ch == mk_lib_fmt_inl_defd_char_c('u')){ mk_lib_fmt_inl_defd_vsnnprintf_llu(dst, rem, va, &gud, &consumed); if(!gud){ break; } mk_lang_assert(consumed >= 1 && consumed <= rem); dst += consumed; rem -= consumed; }
				else if(ch == mk_lib_fmt_inl_defd_char_c('d')){ mk_lib_fmt_inl_defd_vsnnprintf_lld(dst, rem, va, &gud, &consumed); if(!gud){ break; } mk_lang_assert(consumed >= 1 && consumed <= rem); dst += consumed; rem -= consumed; }
				else
				{
					mk_lang_assert(mk_lang_false);
				}
