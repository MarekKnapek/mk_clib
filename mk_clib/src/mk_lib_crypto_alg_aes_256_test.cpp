...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_alg_aes_256_test_encrypt_from_str_lit(char const(&key_lit)[key_lit_len], char const(&msg_lit)[msg_lit_len]) mk_lang_noexcept
{
	...
	for(i = 0; i != mk_lib_crypto_alg_aes_256_msg_len_v; ++i){ ret[i] = msg.m_data.m_uint8s[i]; }
	return ret;
}

template<mk_lang_types_usize_t key_lit_len, typename msg_t>
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_alg_aes_256_test_decrypt_from_str_lit(char const(&key_lit)[key_lit_len], msg_t const& m) mk_lang_noexcept
{
	mk_lib_crypto_alg_aes_256_key_t key mk_lang_constexpr_init;
	mk_lang_types_usize_t i mk_lang_constexpr_init;
	mk_lib_crypto_alg_aes_256_msg_t msg mk_lang_constexpr_init;
	...
	mk_lang_static_assert(key_lit_len == mk_lib_crypto_alg_aes_256_key_len_v * 2 + 1);
	mk_lang_static_assert(msg_t::s_size == mk_lib_crypto_alg_aes_256_msg_len_v);

	mk_lib_cpp_constexpr_hex_str_lit_to_u8s_arr(key.m_data.m_uint8s, key_lit);
	for(i = 0; i != mk_lib_crypto_alg_aes_256_msg_len_v; ++i){ msg.m_data.m_uint8s[i] = m[i]; }
