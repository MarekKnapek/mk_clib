#ifndef mk_include_guard_mk_lib_crypto_mac_hmac_sha0_fuzz_c
#define mk_include_guard_mk_lib_crypto_mac_hmac_sha0_fuzz_c
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_crypto_mac_hmac_sha0_fuzz_compute_my(mk_sl_cui_uint8_pct const key_buf, mk_lang_types_uint_t const key_len, mk_sl_cui_uint8_pct const data_buf, mk_lang_types_uint_t const data_len, mk_lib_crypto_mac_hmac_sha0_digest_pt const digest) mk_lang_noexcept
{
	mk_lib_crypto_mac_hmac_sha0_t hmac;
	...
	mk_lib_crypto_mac_hmac_sha0_init(&hmac, key_buf, key_len);
	mk_lib_crypto_mac_hmac_sha0_append(&hmac, data_buf, data_len);
	mk_lib_crypto_mac_hmac_sha0_finish(&hmac, digest);
	return 0;
}


mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_crypto_mac_hmac_sha0_fuzz(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	mk_lib_crypto_mac_hmac_sha0_digest_t digest_my;
	...
	d += data_len;
	s -= data_len;
	err = mk_lib_crypto_mac_hmac_sha0_fuzz_compute_my(&key_buf[0], key_len, &data_buf[0], data_len, &digest_my); mk_lang_check_rereturn(err);
	return 0;
}
