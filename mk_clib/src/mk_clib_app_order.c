...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_order_print_digest(mk_lib_crypto_hash_stream_any1_id_t const id, mk_sl_cui_uint8_pct const digest) mk_lang_noexcept
{
...
mk_lang_nodiscard static mk_lang_inline mk_lib_crypto_hash_stream_any1_id_t mk_clib_app_order_find_hash(mk_lang_tchar_pct const hash_buf, mk_lang_types_sint_t const hash_len) mk_lang_noexcept
{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_order_work(mk_lang_types_sint_t const argc, mk_lang_tchar_pcpct const argv, mk_lang_types_sint_pct const lens) mk_lang_noexcept
{
	...
	id = mk_clib_app_order_find_hash(arg_hash_buf, arg_hash_len);
	mk_lang_check_return(id != mk_lib_crypto_hash_stream_any1_id_e_dummy_end);
	mk_lib_crypto_hash_stream_any2_init(&hasher, id);
	ptr = ((mk_sl_cui_uint8_pt)(mk_lang_roundup_align(&buff.m_data.m_uchars[0], mk_clib_app_order_buff_algn)));
	err = mk_sl_io_reader_file_open_t(&input_file, arg_file_buf); mk_lang_check_rereturn(err);
	for(;;)
	{
		err = mk_sl_io_reader_file_read(&input_file, ptr, mk_clib_app_order_buff_size, &read); mk_lang_check_rereturn(err);
		if(read == 0)
		{
			break;
		}
	...
	err = mk_clib_app_order_print_digest(id, &digest[0]); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_order_print_algs(mk_lang_types_void_t) mk_lang_noexcept
{
	mk_lang_types_sint_t n;
	mk_lang_types_sint_t i;
	mk_lib_crypto_hash_stream_any1_id_t id;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_order_usage(mk_lang_types_sint_t const argc, mk_lang_tchar_pcpct const argv, mk_lang_types_sint_pct const lens) mk_lang_noexcept
{
	...
	err = mk_clib_app_order_print_algs(); mk_lang_check_rereturn(err);
