...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_crypto_hash_stream_sha1_fuzz_compute_ltc(mk_sl_cui_uint8_pct const data_buf, mk_lang_types_uint_t const data_len, mk_lib_crypto_hash_stream_sha1_digest_pt const digest) mk_lang_noexcept
{
	...
	st = sha1_init(&hasher_ltc); mk_lang_check_return(st == CRYPT_OK);
	st = sha1_process(&hasher_ltc, ((mk_lang_types_uchar_pct)(data_buf)), data_len); mk_lang_check_return(st == CRYPT_OK);
	st = sha1_done(&hasher_ltc, ((mk_lang_types_uchar_pt)(digest))); mk_lang_check_return(st == CRYPT_OK);
	return 0;
}
#endif
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_crypto_hash_stream_sha1_fuzz(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	#if mk_lang_external_ltc_have
	mk_lib_crypto_hash_stream_sha1_digest_t digest_ltc;
	#endif
	#if mk_lang_platform_is_windows_at_least_any
	...
	#if mk_lang_external_ltc_have
	err = mk_lib_crypto_hash_stream_sha1_fuzz_compute_ltc(&data_buf[0], data_len, &digest_ltc); mk_lang_check_rereturn(err);
	mk_lang_test(mk_sl_cui_uint8_memcmp_fn(&digest_my.m_data.m_uint8s[0], &digest_ltc.m_data.m_uint8s[0], mk_lib_crypto_hash_stream_sha1_digest_len_v) == 0);
	#endif
	#if mk_lang_platform_is_windows_at_least_any
