...
mk_lang_extern_c void mk_lib_crypto_hash_stream_sha1x86_test(void) mk_lang_noexcept
{
	#define message_1 ""
	#define message_2 "a"
	...
	int n;
	int i;
	mk_lib_crypto_hash_stream_sha1x86_t sha1;
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lib_crypto_hash_stream_sha1x86_init(&sha1);
		mk_lib_crypto_hash_stream_sha1x86_append(&sha1, ((mk_lang_types_uchar_pct)(s_messages[i])), s_message_lens[i]);
		mk_lib_crypto_hash_stream_sha1x86_finish(&sha1, &digest);
		for(j = 0; j != mk_lib_crypto_hash_block_sha1x86_digest_len; ++j)
		{
