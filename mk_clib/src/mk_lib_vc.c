...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_vc_tweak_next(mk_lib_vc_tweak_pct const a, mk_lib_vc_tweak_pt const b) mk_lang_noexcept
{
	...
	mk_sl_cui_uint8_shr3(&a->m_data.m_uint8s[mk_lib_vc_msg_len - 1], mk_lang_charbit - 1, &tu8);
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_vc_alg_decrypt_block(mk_lib_vc_algid_t const algid, mk_lib_vc_alg_schedule_t const schedule, mk_lib_vc_block_pct const block_in, mk_lib_vc_block_pt const block_out) mk_lang_noexcept
{
	...
	switch(algid)
	{
		case mk_lib_vc_algid_e_aes:     mk_lang_assert(schedule.m_data.m_aes);     mk_lib_crypto_alg_aes_256_schedule_decrypt(schedule.m_data.m_aes,     &block_in->m_data.m_aess[0],     &block_out->m_data.m_aess[0],     mk_lib_vc_msgs_per_block); break;
		case mk_lib_vc_algid_e_serpent: mk_lang_assert(schedule.m_data.m_serpent); mk_lib_crypto_alg_serpent_schedule_decrypt(schedule.m_data.m_serpent, &block_in->m_data.m_serpents[0], &block_out->m_data.m_serpents[0], mk_lib_vc_msgs_per_block); break;
		case mk_lib_vc_algid_e_dummy: mk_lang_assert(mk_lang_false); break;
		default: mk_lang_assert(mk_lang_false); break;
	}
}

mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_vc_xts_decrypt_block(mk_lib_vc_algid_t const algid, mk_lib_vc_alg_schedule_t const schedule_a, mk_lib_vc_alg_schedule_t const schedule_b, mk_sl_cui_uint64_pct const block_id, mk_lib_vc_block_pct const in, mk_lib_vc_block_pt const out) mk_lang_noexcept
{
	...
	mk_lib_vc_alg_encrypt_tweak(algid, schedule_b, &tweaks->m_data.m_tweaks[0], &tweaks->m_data.m_tweaks[0]);
	...
	mk_lib_vc_alg_decrypt_block(algid, schedule_a, out, out);
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_vc_keys_init(mk_lib_vc_seqid_t const seqid, mk_lib_vc_key_material_pct const key_material, mk_lib_vc_seq_keys_pt const keys) mk_lang_noexcept
{
	mk_lang_assert(seqid >= 0 && seqid < mk_lib_vc_seqid_e_dummy);
...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo mk_lang_types_sint_t mk_lib_vc_try_decrypt_header_seq(mk_lib_vc_seqid_t const seqid, mk_lib_vc_key_material_pct const key_material, mk_lib_vc_block_pct const block, mk_lib_vc_seq_schedules_pt const schedules, mk_sl_cui_uint64_pt const volume_len) mk_lang_noexcept
{
	...
	mk_lib_vc_key_material_t master_key_material mk_lang_constexpr_init;
	...
	n = mk_lib_vc_lens_reserved_a;
	for(i = 0; i != n; ++i)
	{
		mk_lang_check_return(mk_sl_cui_uint8_is_zero(&decrypted->m_data.m_uint8s[mk_lib_vc_offsets_reserved_a + i]));
	}
	mk_sl_uint_64_from_8_be(&tu64a, &decrypted->m_data.m_uint8s[mk_lib_vc_offsets_hidden_volume_size]); mk_lang_check_return(mk_sl_cui_uint64_is_zero(&tu64a)); /* todo hidden volumes not supported, yet */
	...
	n = mk_lib_vc_lens_reserved_b;
	for(i = 0; i != n; ++i)
	{
		mk_lang_check_return(mk_sl_cui_uint8_is_zero(&decrypted->m_data.m_uint8s[mk_lib_vc_offsets_reserved_b + i]));
	}
	...
	mk_lib_vc_memcpyu8_fn(&master_key_material.m_data.m_uint8s[0], &decrypted->m_data.m_uint8s[mk_lib_vc_offsets_keys], mk_lang_countof(master_key_material.m_data.m_uint8s));
	mk_lib_vc_keys_init(seqid, &master_key_material, &master_keys);
...
{
	mk_lib_vc_key_material_t key_material mk_lang_constexpr_init;
