...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_hash_stream_blake3_test_compute_from_str_lit(char const(&str_lit)[str_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_cpp_constexpr_array_t<mk_sl_cui_uint8_t, mk_lib_crypto_hash_block_blake3_digest_len> ret mk_lang_constexpr_init;
	...
	mk_lib_crypto_hash_stream_blake3_append_u8(&blake3, msg.data(), msg.size());
	mk_lib_crypto_hash_stream_blake3_finish(&blake3, &digest);
	for(i = 0; i != mk_lib_crypto_hash_block_blake3_digest_len; ++i){ ret[i] = digest.m_uint8s[i]; }
	return ret;
}
