...
{
	mk_lib_mt_atomic_counter_t m_ref_count;
...
struct mk_clib_app_fe_window_s
{
	...
	mk_lang_types_uint_t m_req_id;
	mk_clib_app_fe_server_response_pt m_response_current;
	mk_lang_types_sint_t m_line_height_cur;
	mk_lang_types_sint_t m_line_height_max;
	mk_lang_types_sint_t m_idx;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_server_response_destroy(mk_clib_app_fe_server_response_pt const response) mk_lang_noexcept
{
	...
	err = mk_lib_mt_atomic_counter_destruct(&response->m_ref_count); mk_lang_check_rereturn(err);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_server_response_ref_count_dec(mk_clib_app_fe_server_pt const server, mk_clib_app_fe_server_response_pt const response) mk_lang_noexcept
{
	...
	err = mk_lib_mt_atomic_counter_decrement(&response->m_ref_count, &cmp); mk_lang_check_rereturn(err); mk_lang_assert(cmp >= 0);
	if(cmp == 0)
	{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_server_request_process_root_impl_init(mk_clib_app_fe_server_request_pt const request, mk_clib_app_fe_server_response_pt const response) mk_lang_noexcept
{
	...
	err = mk_lib_mt_atomic_counter_construct(&response->m_ref_count); mk_lang_check_rereturn(err);
	err = mk_lib_mt_atomic_counter_increment(&response->m_ref_count, &cmp); mk_lang_check_rereturn(err); mk_lang_assert(cmp > 0);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_server_request_ref_count_dec(mk_clib_app_fe_server_pt const server, mk_clib_app_fe_server_request_pt const request) mk_lang_noexcept
{
	...
	err = mk_lib_mt_atomic_counter_decrement(&request->m_ref_count, &cmp); mk_lang_check_rereturn(err); mk_lang_assert(cmp >= 0);
	if(cmp == 0)
	{
		err = mk_lib_mt_atomic_counter_destruct(&request->m_ref_count); mk_lang_check_rereturn(err);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_scroll_to_selected(mk_clib_app_fe_window_pt const window) mk_lang_noexcept
{
	...
	if(window->m_response_current)
	{
		...
		client_whole_rows = client_height / window->m_line_height_cur;
		idx = window->m_idx;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_draw_response_root_widest_drive(mk_clib_app_fe_window_pt const window, mk_lang_types_sint_pt const widest) mk_lang_noexcept
{
	...
	mk_win_base_sint_t line_height_max;
	...
	for(i = 0; i != n; ++i)
	{
		if((drives & 0x1) != 0)
		{
			...
			line_height_max = mk_lang_max(line_height_max, ((mk_win_base_sint_t)(recta.m_bottom - recta.m_top)));
		}
		drives >>= 1;
	}
	font_old = mk_win_user_dc_select_font(dc, font_old); mk_lang_assert(font_old.m_data == window->m_font.m_data);
	window->m_line_height_max = mk_lang_max(window->m_line_height_max, line_height_max);
	*widest = width;
	return 0;
}
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_draw_response_root(mk_clib_app_fe_window_pt const window) mk_lang_noexcept
{
	...
	mk_lang_types_sint_t line_height_max;
	mk_lang_types_sint_t err;
	mk_win_user_font_t font_old;
	...
	for(i = window->m_scroll; i != n; ++i)
	{
		if((drives & 0x1) != 0)
		{
			...
			err = mk_clib_app_fe_window_on_paint_impl_draw_response_root_line_height(window, i, &line_height); mk_lang_check_rereturn(err); line_height_max = mk_lang_max(line_height_max, line_height);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_rows_root_sizes(mk_clib_app_fe_window_paint_pt const pt, mk_lang_types_sint_pt const out_width) mk_lang_noexcept
{
	...
	window->m_line_height_max = mk_lang_max(window->m_line_height_max, height_max);
	*out_width = width_max;
	return 0;
}
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_row_selected_on(mk_clib_app_fe_window_paint_pt const pt, mk_lang_types_sint_t const j) mk_lang_noexcept
{
	...
	if(j == window->m_idx)
	{
		...
		old_bg = mk_win_user_dc_set_bk_color(dc, mk_win_user_base_get_sys_color(((mk_win_base_sint_t)(mk_win_user_base_color_id_e_highlight)))); mk_lang_check_return(old_bg != ((mk_win_user_base_colorref_t)(0xfffffffful)));
		rect = pt->m_rect;
		rect.m_bottom = pt->m_rect.m_top + window->m_line_height_cur;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_rows_root(mk_clib_app_fe_window_paint_pt const pt) mk_lang_noexcept
{
	...
	dc = window->m_mem_dc;
	resp = window->m_response_current;
	line_height = window->m_line_height_cur;
	line_height_max = 0;
	response = &resp->m_data.m_root;
	drive_text[1] = L':';
	drives = response->m_drive_letters;
	...
	for(i = 0; i != n; ++i)
	{
		if((drives & 0x1u) != 0)
		{
			...
			recta = *rect;
			height = mk_win_user_dc_draw_text_w(dc, &drive_text[0], mk_lang_countof(drive_text), &recta, format); mk_lang_check_return(height != 0); line_height_max = mk_lang_max(line_height_max, height);
			recta = *rect; recta.m_left += 2 * width;
			height = mk_win_user_dc_draw_text_w(dc, &drive_type_buf[0], drive_type_len, &recta, format); mk_lang_check_return(height != 0); line_height_max = mk_lang_max(line_height_max, height);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_rows_idx_erry(mk_clib_app_fe_window_paint_pt const pt) mk_lang_noexcept
{
	...
	window->m_line_height_max = mk_lang_max(window->m_line_height_max, height);
	return 0;
}
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_up(mk_clib_app_fe_window_paint_pt const pt) mk_lang_noexcept
{
	...
	if(has_up && scroll == 0)
	{
		...
		font_old = mk_win_user_dc_select_font(dc, window->m_font);
		height = mk_win_user_dc_draw_text_w(dc, &text_buf[0], text_len, &recta, format); mk_lang_check_return(height != 0); line_height_max = mk_lang_max(line_height_max, height);
		font_old = mk_win_user_dc_select_font(dc, font_old); mk_lang_assert(font_old.m_data == window->m_font.m_data);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_make_request_root_and_wait(mk_clib_app_fe_window_pt const window) mk_lang_noexcept
{
	mk_lang_types_sint_t cmp;
	mk_clib_app_fe_server_request_pt request;
	mk_lang_types_sint_t err;
	...
	err = mk_lib_mt_atomic_counter_construct(&request->m_ref_count); mk_lang_check_rereturn(err);
	err = mk_lib_mt_atomic_counter_increment(&request->m_ref_count, &cmp); mk_lang_check_rereturn(err); mk_lang_assert(cmp > 0);
	...
	err = mk_clib_app_fe_server_responsesmt_rw_pop_front_copy(&window->m_responses_waiting, &response); mk_lang_check_rereturn(err); mk_lang_assert(response);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_on_paint_impl_2(mk_clib_app_fe_window_pt const window) mk_lang_noexcept
{
	...
	if(window->m_init)
	{
		...
		err = mk_clib_app_fe_window_make_request_root_and_wait(window); mk_lang_check_rereturn(err);
	}
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_window_construct(mk_clib_app_fe_window_pt const window, mk_clib_app_fe_server_pt const server) mk_lang_noexcept
{
	...
	window->m_line_height_cur = 1;
	window->m_line_height_max = 1;
	window->m_idx = 0;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_fe_on_idle(mk_clib_app_fe_pt const fe) mk_lang_noexcept
{
	mk_win_base_bool_t b;

	mk_lang_assert(fe);

	if(fe->m_window.m_line_height_cur < fe->m_window.m_line_height_max)
	{
		fe->m_window.m_line_height_cur = fe->m_window.m_line_height_max;
		b = mk_win_user_window_invalidate_region(fe->m_window.m_wnd, mk_win_user_region_get_null(), mk_win_base_true); mk_lang_check_return(b != 0);
	}
	return 0;
}
