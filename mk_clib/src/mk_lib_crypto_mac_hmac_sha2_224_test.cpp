#ifndef mk_include_guard_mk_lib_crypto_mac_hmac_sha2_224_test_cpp
#define mk_include_guard_mk_lib_crypto_mac_hmac_sha2_224_test_cpp
...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit(mk_lang_types_pchar_t const(&msg_str_lit)[msg_str_lit_len], mk_lang_types_pchar_t const(&key_str_lit)[key_str_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_crypto_mac_hmac_sha2_224_t hmac mk_lang_constexpr_init;
	mk_lib_crypto_mac_hmac_sha2_224_digest_t digest mk_lang_constexpr_init;
	mk_lib_cpp_constexpr_array_t<mk_sl_cui_uint8_t, mac_len> ret mk_lang_constexpr_init;

	mk_lang_static_assert(mac_len >= 1);
	mk_lang_static_assert(mac_len <= mk_lib_crypto_mac_hmac_sha2_224_digest_len_v);
	...
	mk_lib_crypto_mac_hmac_sha2_224_init(&hmac, key.data(), key.size());
	mk_lib_crypto_mac_hmac_sha2_224_append(&hmac, msg.data(), msg.size());
...
mk_lang_extern_force_c mk_lang_types_void_t mk_lib_crypto_mac_hmac_sha2_224_test(mk_lang_types_void_t) mk_lang_noexcept
{
	...
	#define key_2 "000102030405060708090a0b0c0d0e0f101112131415161718191a1b"
	...
	#define mac_1 "c7405e3ae058e8cd30b08b4140248581ed174cb34e1224bcc1efc81b"
	#define mac_2 "e3d249a8cfb67ef8b7a169e9a0a599714a2cecba65999a51beb8fbbe"
	#define mac_3 "91c52509e5af8531601ae6230099d90bef88aaefb961f4080abc014d"
	#define mac_4 "d522f1df596ca4b4b1c23d27bde067d6"

	#if mk_lang_version_at_least_cpp_14 || mk_lang_version_at_least_msvc_cpp_14

	mk_lang_constexpr_static auto const s_mac_computed_1 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit<mk_lang_countstr(mac_1) / 2>(msg_1, key_1);
	mk_lang_constexpr_static auto const s_mac_computed_2 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit<mk_lang_countstr(mac_2) / 2>(msg_2, key_2);
	mk_lang_constexpr_static auto const s_mac_computed_3 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit<mk_lang_countstr(mac_3) / 2>(msg_3, key_3);
	mk_lang_constexpr_static auto const s_mac_computed_4 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit<mk_lang_countstr(mac_4) / 2>(msg_4, key_4);
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lang_assert(mac_lens[i] <= mk_lib_crypto_mac_hmac_sha2_224_digest_len_v);
		...
		mk_lib_crypto_mac_hmac_sha2_224_init(&hmac, &key[0], ((mk_lang_types_usize_t)(key_lens[i])));
		mk_lib_crypto_mac_hmac_sha2_224_append(&hmac, ((mk_sl_cui_uint8_pct)(msgs[i])), ((mk_lang_types_usize_t)(msg_lens[i])));
		mk_lib_crypto_mac_hmac_sha2_224_finish(&hmac, &digest);
		m = mac_lens[i];
		for(j = 0; j != m; ++j)
		{
