...
template<int mac_len = mk_lib_crypto_hash_block_sha2_224_digest_len, int msg_len, int key_len>
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit(mk_lang_types_pchar_t const(&msg)[msg_len], mk_lang_types_pchar_t const(&key)[key_len]) mk_lang_noexcept
{
	...
	mk_lib_crypto_mac_hmac_sha2_224_init(&hmac, k.data(), k.size());
	mk_lib_crypto_mac_hmac_sha2_224_append(&hmac, m.data(), m.size());
...
mk_lang_extern_c void mk_lib_crypto_mac_hmac_sha2_224_test(void) mk_lang_noexcept
{
	...
	mk_lang_constexpr_static auto const s_key_computed_1 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit(msg_1, key_1);
	mk_lang_constexpr_static auto const s_key_computed_2 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit(msg_2, key_2);
	mk_lang_constexpr_static auto const s_key_computed_3 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit(msg_3, key_3);
	mk_lang_constexpr_static auto const s_key_computed_4 = mk_lib_crypto_mac_hmac_sha2_224_test_compute_from_str_lit<16>(msg_4, key_4);
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lib_crypto_mac_hmac_sha2_224_init(&hmac, &key[0], s_key_lens[i]);
		mk_lib_crypto_mac_hmac_sha2_224_append(&hmac, &msg[0], s_msg_lens[i]);
		mk_lib_crypto_mac_hmac_sha2_224_finish(&hmac, &digest);
		m = s_mac_lens[i];
		for(j = 0; j != m; ++j)
		{
