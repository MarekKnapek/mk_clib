...
#define mk_sl_tree_wavl_t_mallocatorg_name mk_sl_mallocatorg_portablec
...
#define mk_sl_vector_t_mallocatorg mk_sl_mallocatorg_portablec
...
mk_lang_jumbo mk_lang_types_void_t mk_sl_tree_wavl_fuzz(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	d = data;
	s = size;
	err = mk_sl_mallocatorg_portablec_init(); mk_lang_check_recrash(err);
	...
	for(;;)
	{
		...
		for(j = 0; j != m; ++j)
		{
			err = mk_sl_tree_wavl_fuzz_test_ro_find(&tree, mk_sl_tree_wavl_fuzz_vec_ro_at(&vec, j), &node); mk_lang_test(err == 0); mk_lang_test(node); mk_lang_test(mk_sl_tree_wavl_fuzz_test_ro_node_get_element(node, &pval) == 0 && *pval == *mk_sl_tree_wavl_fuzz_vec_ro_at(&vec, j));
		}
		if(!(s >= 2)) break;
		n = ((mk_lang_types_sint_t)(((((mk_lang_types_uint_t)(mk_lang_types_uint_t)(d[0])) << 8)) | ((mk_lang_types_uint_t)(d[1])))); d += 2; s -= 2;
		for(i = 0; i != n; ++i)
		{
			...
			if(j != m)
			{
				err = mk_sl_tree_wavl_fuzz_test_ro_find(&tree, &val, &node); mk_lang_test(err == 0); mk_lang_test(node); mk_lang_test(mk_sl_tree_wavl_fuzz_test_ro_node_get_element(node, &pval) == 0 && *pval == val);
			...
			{
				err = mk_sl_tree_wavl_fuzz_test_ro_find(&tree, &val, &node); mk_lang_test(err == 0); mk_lang_test(!node);
			}
		}
	}
	err = mk_sl_tree_wavl_fuzz_vec_rw_destroy(&vec); mk_lang_check_recrash(err);
	err = mk_sl_tree_wavl_fuzz_test_rw_destruct(&tree); mk_lang_check_recrash(err);
	err = mk_sl_mallocatorg_portablec_deinit(); mk_lang_check_recrash(err);
}
