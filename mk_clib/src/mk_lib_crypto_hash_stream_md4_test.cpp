...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_hash_stream_md4_test_compute_from_str_lit(char const(&str_lit)[str_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_crypto_hash_block_md4_digest_t digest mk_lang_constexpr_init;
	mk_lang_types_usize_t i mk_lang_constexpr_init;
	mk_lib_cpp_constexpr_array_t<mk_sl_cui_uint8_t, mk_lib_crypto_hash_block_md4_digest_len> ret mk_lang_constexpr_init;

	mk_lang_static_assert(str_lit_len >= 1);

	msg = mk_lib_cpp_constexpr_str_lit_to_u8s(str_lit);
	mk_lib_crypto_hash_stream_md4_init(&md4);
	mk_lib_crypto_hash_stream_md4_append_u8(&md4, msg.data(), msg.size());
	mk_lib_crypto_hash_stream_md4_finish(&md4, &digest);
	for(i = 0; i != mk_lib_crypto_hash_block_md4_digest_len; ++i){ ret[i] = digest.m_uint8s[i]; }
	return ret;
}
...
mk_lang_extern_c void mk_lib_crypto_hash_stream_md4_test(void) mk_lang_noexcept
{
	#define message_1 ""
	#define message_2 "a"
	...
	int n;
	int i;
	mk_lib_crypto_hash_stream_md4_t md4;
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lib_crypto_hash_stream_md4_init(&md4);
		mk_lib_crypto_hash_stream_md4_append(&md4, ((mk_lang_types_uchar_pct)(s_messages[i])), s_message_lens[i]);
		mk_lib_crypto_hash_stream_md4_finish(&md4, &digest);
		for(j = 0; j != mk_lib_crypto_hash_block_md4_digest_len; ++j)
		{
