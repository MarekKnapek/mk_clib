#ifndef mk_include_guard_mk_lib_crypto_hash_stream_md4_test_cpp
#define mk_include_guard_mk_lib_crypto_hash_stream_md4_test_cpp
...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_hash_stream_md4_test_compute_from_str_lit(mk_lang_types_pchar_t const(&str_lit)[str_lit_len]) mk_lang_noexcept
{
	mk_lib_cpp_constexpr_array_t<mk_sl_cui_uint8_t, str_lit_len - 1> msg mk_lang_constexpr_init;
	mk_lib_crypto_hash_stream_md4_t md4 mk_lang_constexpr_init;
	mk_lib_crypto_hash_stream_md4_digest_t digest mk_lang_constexpr_init;
	mk_lib_cpp_constexpr_array_t<mk_sl_cui_uint8_t, mk_lib_crypto_hash_stream_md4_digest_len_v> ret mk_lang_constexpr_init;

	mk_lang_static_assert(str_lit_len >= 1);

	msg = mk_lib_cpp_constexpr_array_u8_from_pchar_lit(str_lit);
	mk_lib_crypto_hash_stream_md4_init(&md4);
	mk_lib_crypto_hash_stream_md4_append_u8s(&md4, msg.data(), msg.size());
	mk_lib_crypto_hash_stream_md4_finish(&md4, &digest);
...
mk_lang_extern_force_c mk_lang_inline mk_lang_types_void_t mk_lib_crypto_hash_stream_md4_test(mk_lang_types_void_t) mk_lang_noexcept
{
	#define message_1 ""
	#define message_2 "a"
	...
	#define digest_3 "a448017aaf21d8525fc10ae87aa6729d"
	#define digest_4 "d9130a8164549fe818874806e1c7014b"
	...
	#define digest_8 "bd5b8b581f6ece4146c588fdeea041f0"

	#if mk_lang_version_at_least_cpp_14 || mk_lang_version_at_least_msvc_cpp_14

	mk_lang_constexpr_static auto const s_digest_computed_1 = mk_lib_crypto_hash_stream_md4_test_compute_from_str_lit(message_1);
	mk_lang_constexpr_static auto const s_digest_computed_2 = mk_lib_crypto_hash_stream_md4_test_compute_from_str_lit(message_2);
	mk_lang_constexpr_static auto const s_digest_computed_3 = mk_lib_crypto_hash_stream_md4_test_compute_from_str_lit(message_3);
	mk_lang_constexpr_static auto const s_digest_computed_4 = mk_lib_crypto_hash_stream_md4_test_compute_from_str_lit(message_4);
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lib_crypto_hash_stream_md4_init(&md4);
		mk_lib_crypto_hash_stream_md4_append_u8s(&md4, ((mk_sl_cui_uint8_pct)(messages[i])), ((mk_lang_types_usize_t)(message_lens[i])));
		mk_lib_crypto_hash_stream_md4_finish(&md4, &digest);
		for(j = 0; j != mk_lib_crypto_hash_stream_md4_digest_len_v; ++j)
		{
