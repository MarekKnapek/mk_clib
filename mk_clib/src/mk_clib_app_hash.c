#ifndef mk_include_guard_mk_clib_app_hash_c
#define mk_include_guard_mk_clib_app_hash_c
...
#define mk_clib_app_hash_buff_size 4 * 1024
#define mk_clib_app_hash_buff_algn 4 * 1024
union mk_clib_app_hash_buff_data_u
{
	mk_lang_types_uchar_t m_uchars[mk_lang_roundup_add(mk_clib_app_hash_buff_size, mk_clib_app_hash_buff_algn)];
	mk_lang_types_ulllong_t m_ulllong;
};
typedef union mk_clib_app_hash_buff_data_u mk_clib_app_hash_buff_data_t;
struct mk_clib_app_hash_buff_s
{
	mk_clib_app_hash_buff_data_t m_data;
};
typedef struct mk_clib_app_hash_buff_s mk_clib_app_hash_buff_t;
typedef mk_clib_app_hash_buff_t* mk_clib_app_hash_buff_pt;


static mk_lang_inline mk_lang_types_void_t mk_clib_app_hash_usage_strs(mk_lang_types_void_t) mk_lang_noexcept
{
...
static mk_lang_inline mk_lang_types_void_t mk_clib_app_hash_print(mk_lib_crypto_hash_stream_any1_id_t const id, mk_sl_cui_uint8_pct const digest) mk_lang_noexcept
{
	mk_lang_types_sint_t n;
	mk_lang_types_sint_t i;
	mk_lang_types_sint_t len;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_pchar_t mk_clib_app_hash_to_lower(mk_lang_types_pchar_t const pc) mk_lang_noexcept
{
	...
	mk_lang_static_assert(mk_lang_countof(s_mk_clib_app_hash_uc) == mk_lang_countof(s_mk_clib_app_hash_lc));

	res = pc;
	n = mk_lang_countof(s_mk_clib_app_hash_uc);
	for(i = 0; i != n; ++i)
	{
		if(pc == s_mk_clib_app_hash_uc[i])
		{
			res = s_mk_clib_app_hash_lc[i];
			break;
		}
	}
	return res;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_hash_work(mk_lang_types_sint_t const argc, mk_lang_types_pchar_pct const* const argv) mk_lang_noexcept
{
	...
	ptr = ((mk_sl_cui_uint8_pt)(mk_lang_roundup_align(&buff.m_data.m_uchars[0], mk_clib_app_hash_buff_algn)));
	inputf = fopen(v[2], "rb"); mk_lang_check_return(inputf);
	for(;;)
	{
		read = fread(ptr, 1, mk_clib_app_hash_buff_size, inputf);
		if(read == 0)
		{
			break;
		}
	...
	mk_clib_app_hash_print(id, &digest[0]);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_clib_app_hash_usage(mk_lang_types_sint_t const argc, mk_lang_types_pchar_pcpct const argv) mk_lang_noexcept
{
...
mk_lang_extern_c mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_clib_app_hash_args(mk_lang_types_sint_t const argc, mk_lang_types_pchar_pcpct const argv) mk_lang_noexcept
{
	...
	if(argc == 1)
	{
		err = mk_clib_app_hash_usage(argc, argv); mk_lang_check_rereturn(err);
	}
	else
	{
		mk_lang_check_return(argc == 3);
		err = mk_clib_app_hash_work(argc, argv); mk_lang_check_rereturn(err);
	}
	return 0;
}
