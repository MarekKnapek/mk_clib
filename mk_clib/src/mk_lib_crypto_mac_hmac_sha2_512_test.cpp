#ifndef mk_include_guard_mk_lib_crypto_mac_hmac_sha2_512_test_cpp
#define mk_include_guard_mk_lib_crypto_mac_hmac_sha2_512_test_cpp
...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_mac_hmac_sha2_512_test_compute_from_str_lit(mk_lang_types_pchar_t const(&msg_str_lit)[msg_str_lit_len], mk_lang_types_pchar_t const(&key_str_lit)[key_str_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_crypto_mac_hmac_sha2_512_digest_t digest mk_lang_constexpr_init;
	mk_lib_cpp_constexpr_array_t<mk_sl_cui_uint8_t, mac_len> ret mk_lang_constexpr_init;

	mk_lang_static_assert(mac_len >= 1);
	mk_lang_static_assert(mac_len <= mk_lib_crypto_mac_hmac_sha2_512_digest_len_v);
	...
	mk_lib_crypto_mac_hmac_sha2_512_init(&hmac, key.data(), key.size());
	mk_lib_crypto_mac_hmac_sha2_512_append(&hmac, msg.data(), msg.size());
...
mk_lang_extern_force_c mk_lang_types_void_t mk_lib_crypto_mac_hmac_sha2_512_test(mk_lang_types_void_t) mk_lang_noexcept
{
	...
	mk_lang_constexpr_static auto const s_mac_computed_1 = mk_lib_crypto_mac_hmac_sha2_512_test_compute_from_str_lit<mk_lang_countstr(mac_1) / 2>(msg_1, key_1);
	mk_lang_constexpr_static auto const s_mac_computed_2 = mk_lib_crypto_mac_hmac_sha2_512_test_compute_from_str_lit<mk_lang_countstr(mac_2) / 2>(msg_2, key_2);
	mk_lang_constexpr_static auto const s_mac_computed_3 = mk_lib_crypto_mac_hmac_sha2_512_test_compute_from_str_lit<mk_lang_countstr(mac_3) / 2>(msg_3, key_3);
	mk_lang_constexpr_static auto const s_mac_computed_4 = mk_lib_crypto_mac_hmac_sha2_512_test_compute_from_str_lit<mk_lang_countstr(mac_4) / 2>(msg_4, key_4);
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lang_assert(mac_lens[i] <= mk_lib_crypto_mac_hmac_sha2_512_digest_len_v);
		...
		mk_lib_crypto_mac_hmac_sha2_512_init(&hmac, &key[0], ((mk_lang_types_usize_t)(key_lens[i])));
		mk_lib_crypto_mac_hmac_sha2_512_append(&hmac, ((mk_sl_cui_uint8_pct)(msgs[i])), ((mk_lang_types_usize_t)(msg_lens[i])));
		mk_lib_crypto_mac_hmac_sha2_512_finish(&hmac, &digest);
		m = mac_lens[i];
		for(j = 0; j != m; ++j)
		{
