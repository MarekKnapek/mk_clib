...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_destroy(mk_lib_iip_cp_client_local_listener_task_pt const task) mk_lang_noexcept
{
	...
	mk_lang_assert(mk_lib_iip_cp_client_local_client_tasks_rw_is_empty(&task->m_local.m_state.m_clients));
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_want_associate_socket(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_gimme_socket(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lib_net_socket_ppt const socket, mk_lib_iip_any_data_connection_ppt const associatee) mk_lang_noexcept
{
	...
	mk_lang_assert(mk_lib_iip_cp_client_local_listener_task_prrw_want_associate_socket(task, &want) == 0 && want);
	...
	{
		err = mk_lib_iip_cp_client_local_client_task_rw_gimme_socket(task->m_local.m_state.m_client_want_associate, socket, associatee); mk_lang_check_rereturn(err);
		task->m_local.m_state.m_client_want_associate = mk_lang_null;
	}
	else
	{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_on_iorp_done_find_client(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_void_pt const overlapped, mk_lib_iip_any_data_connection_ppt const client) mk_lang_noexcept
{
	...
	for(i = 0; i != n; ++i)
	{
		...
		err = mk_lib_iip_cp_client_local_client_task_rw_want_irp(client_val, overlapped, &want); mk_lang_check_rereturn(err);
		if(want)
		{
			*client = mk_lib_iip_cp_client_local_client_task_rw_to_any(client_val);
			break;
		}
	}
	mk_lang_assert(i != n);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_on_iorp_done(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const successful_io_operation, mk_lang_types_sint_t const bytes_transferred, mk_lang_types_void_pt const overlapped) mk_lang_noexcept
{
	...
	err = mk_lib_iip_cp_client_local_listener_task_prrw_on_iorp_done_find_client(task, overlapped, &client); mk_lang_check_rereturn(err); mk_lang_assert(client);
	err = mk_lib_iip_cp_client_local_client_task_any_rw_on_iorp_done(client, successful_io_operation, bytes_transferred, overlapped); mk_lang_check_rereturn(err);
	return 0;
}
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step_create_client(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	...
	mk_lang_types_sint_t err;
	mk_lib_iip_any_data_connection_pt client_any;
	mk_lib_iip_cp_client_local_client_task_pt client_normal;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step_idle_no_stop(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	...
	mk_lang_types_bool_t want;
	mk_lib_iip_cp_client_local_client_task_pt want_die;
	mk_lang_types_usize_t n;
	mk_lang_types_usize_t i;
	...
	for(i = 0; i != n; ++i)
	{
		...
		switch(stp_res)
		{
			...
			{
				err = mk_lib_iip_cp_client_local_client_task_rw_want_associate_socket(client_val, &want); mk_lang_check_rereturn(err);
				if(want)
				{
					task->m_local.m_state.m_client_want_associate = client_val;
				}
				if(!task->m_local.m_state.m_client_want_associate && !want_die)
				{
					err = mk_lib_iip_cp_client_local_client_task_rw_want_die(client_val, &want); mk_lang_check_rereturn(err);
					if(want)
					{
						want_die = client_val;
					}
	...
	if(i == n)
	{
		...
		else
		{
			if(want_die)
			{
				err = mk_lib_iip_cp_client_local_client_tasks_rw_erase_value(&task->m_local.m_state.m_clients, &want_die); mk_lang_check_rereturn(err);
				*step_result = mk_lib_iip_cp_client_local_listener_task_result_e_did_something;
			}
			else
			{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step_idle(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	...
	if(task->m_local.m_state.m_stop_requested && task->m_local.m_state.m_stop_acknowledged && *step_result == mk_lib_iip_cp_client_local_listener_task_result_e_did_nothing)
	{
		if(mk_lib_iip_cp_client_local_client_tasks_rw_is_empty(&task->m_local.m_state.m_clients))
		{
			task->m_step = mk_lib_iip_cp_client_local_listener_task_step_e_want_die;
			*step_result = mk_lib_iip_cp_client_local_listener_task_result_e_did_something;
		}
	}
	return 0;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step_want_die(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	mk_lang_assert(task);
	mk_lang_assert(allow_to_block == mk_lang_true || allow_to_block == mk_lang_false);
	mk_lang_assert((tm >= 0 && tm <= 10 * 60 * 1000) || tm == -1);
	mk_lang_assert(step_result);
	mk_lang_assert(*step_result == mk_lib_iip_cp_client_local_listener_task_result_e_dummy_end);
	mk_lang_assert(task->m_step == mk_lib_iip_cp_client_local_listener_task_step_e_want_die);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	...
	switch(task->m_step)
	{
		...
		case mk_lib_iip_cp_client_local_listener_task_step_e_want_die             : err = mk_lib_iip_cp_client_local_listener_task_prrw_step_want_die             (task, allow_to_block, tm, step_result); mk_lang_check_rereturn(err); mk_lang_assert(*step_result != mk_lib_iip_cp_client_local_listener_task_result_e_dummy_end); break;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_want_die(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
	mk_lang_types_bool_t wand;
	...
	wand = task->m_step == mk_lib_iip_cp_client_local_listener_task_step_e_want_die;
	*want = wand;
	return 0;
}


mk_lang_nodiscard static mk_lang_inline mk_lib_iip_cp_client_local_listener_task_pt mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(mk_lib_iip_any_data_connection_pt const connection) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_settings_pt settings;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_allocate(mk_lib_iip_any_data_connection_ppt const connection) mk_lang_noexcept
{
	mk_lang_types_sint_t err;
	...
	err = mk_lib_iip_cp_mallocator_global_allocate(sizeof(*self), &mem); mk_lang_check_rereturn(err); mk_lang_assert(mem); self = ((mk_lib_iip_cp_client_local_listener_task_pt)(mem)); mk_lang_assert(self);
	self->m_local.m_settings.m_type.m_data.m_id = mk_lib_iip_any_data_connection_type_e_mk_lib_iip_cp_client_local_listener_task;
	*connection = &self->m_local.m_settings.m_type;
	mk_lang_assert(mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(*connection) == self);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_deallocate(mk_lib_iip_any_data_connection_pt const connection) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_mallocator_global_deallocate(self, sizeof(*self)); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_construct(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_void_pt const settings) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_construct(self, ((mk_lib_iip_cp_client_local_listener_task_settings_pct)(settings))); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_destroy(mk_lib_iip_any_data_connection_pt const connection) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_destroy(self); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_request_close(mk_lib_iip_any_data_connection_pt const connection) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_request_close(self); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_want_associate_socket(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_want_associate_socket(self, want); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_gimme_socket(mk_lib_iip_any_data_connection_pt const connection, mk_lib_net_socket_ppt const socket, mk_lib_iip_any_data_connection_ppt const associatee) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_gimme_socket(self, socket, associatee); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_on_iorp_done(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_t const successful_io_operation, mk_lang_types_sint_t const bytes_transferred, mk_lang_types_void_pt const overlapped) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_on_iorp_done(self, successful_io_operation, bytes_transferred, overlapped); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_step(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_any_data_connection_result_pt const step_result) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_step(self, allow_to_block, tm, ((mk_lib_iip_cp_client_local_listener_task_result_pt)(step_result))); mk_lang_check_rereturn(err);
	return 0;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_want_die(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
	mk_lib_iip_cp_client_local_listener_task_pt self;
	mk_lang_types_sint_t err;
	...
	self = mk_lib_iip_cp_client_local_listener_task_prrw_type_to_self(connection); mk_lang_assert(self);
	err = mk_lib_iip_cp_client_local_listener_task_prrw_want_die(self, want); mk_lang_check_rereturn(err);
	return 0;
}
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_rw_want_associate_socket(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_prrw_want_associate_socket(task, want);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_rw_gimme_socket(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lib_net_socket_ppt const socket, mk_lib_iip_any_data_connection_ppt const associatee) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_prrw_gimme_socket(task, socket, associatee);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_rw_on_iorp_done(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const successful_io_operation, mk_lang_types_sint_t const bytes_transferred, mk_lang_types_void_pt const overlapped) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_prrw_on_iorp_done(task, successful_io_operation, bytes_transferred, overlapped);
}
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_rw_want_die(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_prrw_want_die(task, want);
}


mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_allocate(mk_lib_iip_any_data_connection_ppt const connection) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_allocate(connection);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_deallocate(mk_lib_iip_any_data_connection_pt const connection) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_deallocate(connection);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_construct(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_void_pt const settings) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_construct(connection, settings);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_destroy(mk_lib_iip_any_data_connection_pt const connection) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_destroy(connection);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_request_close(mk_lib_iip_any_data_connection_pt const connection) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_request_close(connection);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_want_associate_socket(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_want_associate_socket(connection, want);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_gimme_socket(mk_lib_iip_any_data_connection_pt const connection, mk_lib_net_socket_ppt const socket, mk_lib_iip_any_data_connection_ppt const associatee) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_gimme_socket(connection, socket, associatee);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_on_iorp_done(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_t const successful_io_operation, mk_lang_types_sint_t const bytes_transferred, mk_lang_types_void_pt const overlapped) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_on_iorp_done(connection, successful_io_operation, bytes_transferred, overlapped);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_step(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_any_data_connection_result_pt const step_result) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_step(connection, allow_to_block, tm, step_result);
}

mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_rw_want_die(mk_lib_iip_any_data_connection_pt const connection, mk_lang_types_bool_pt const want) mk_lang_noexcept
{
	return mk_lib_iip_cp_client_local_listener_task_any_prrw_want_die(connection, want);
}
