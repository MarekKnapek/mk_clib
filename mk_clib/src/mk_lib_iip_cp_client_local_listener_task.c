...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_construct(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lib_iip_cp_client_local_listener_task_settings_pct const settings) mk_lang_noexcept
{
	...
	err = mk_lib_iip_any_data_connections_rw_construct(&task->m_local.m_state.m_clients); mk_lang_check_rereturn(err);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_destroy(mk_lib_iip_cp_client_local_listener_task_pt const task) mk_lang_noexcept
{
	...
	err = mk_lib_iip_any_data_connections_rw_destroy(&task->m_local.m_state.m_clients); mk_lang_check_rereturn(err);
	return 0;
}
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_gimme_socket(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lib_net_socket_ppt const socket, mk_lib_iip_any_data_connection_ppt const associatee) mk_lang_noexcept
{
	...
	{
		err = mk_lib_iip_cp_client_local_client_task_any_rw_gimme_socket(task->m_local.m_state.m_client_want_associate, socket, associatee); mk_lang_check_rereturn(err);
		task->m_local.m_state.m_client_want_associate = mk_lang_null;
	}
	else
	{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_on_iorp_done_find_client(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_void_pt const overlapped, mk_lib_iip_any_data_connection_ppt const client) mk_lang_noexcept
{
	...
	mk_lib_iip_any_data_connection_ppt client_ptr;
	mk_lib_iip_any_data_connection_pt client_val;
	mk_lang_types_sint_t err;
	mk_lang_types_bool_t want;
	...
	n = mk_lib_iip_any_data_connections_rw_size(&task->m_local.m_state.m_clients);
	for(i = 0; i != n; ++i)
	{
		client_ptr = mk_lib_iip_any_data_connections_rw_at(&task->m_local.m_state.m_clients, i); mk_lang_assert(client_ptr); client_val = *client_ptr; mk_lang_assert(client_val);
		err = mk_lib_iip_cp_client_local_client_task_any_rw_want_irp(client_val, overlapped, &want); mk_lang_check_rereturn(err);
		if(want)
		{
			*client = client_val;
			break;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step_create_client(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	...
	err = mk_lib_iip_cp_client_local_client_task_any_rw_allocate(&client); mk_lang_check_rereturn(err);
	err = mk_lib_iip_cp_client_local_client_task_any_rw_construct(client, &settings); mk_lang_check_rereturn(err);
	err = mk_lib_iip_any_data_connections_rw_push_back_move_single(&task->m_local.m_state.m_clients, &client); mk_lang_check_rereturn(err);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step_idle_no_stop(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	...
	for(i = 0; i != n; ++i)
	{
		...
		err = mk_lib_iip_cp_client_local_client_task_any_rw_step(client_val, allow_to_block, tm, &stp_res); mk_lang_check_rereturn(err); mk_lang_assert(stp_res != mk_lib_iip_any_data_connection_result_e_dummy_end);
		switch(stp_res)
		{
			...
			{
				err = mk_lib_iip_cp_client_local_client_task_any_rw_want_associate_socket(client_val, &want); mk_lang_check_rereturn(err);
				if(want)
				{
				...
				{
					err = mk_lib_iip_cp_client_local_client_task_any_rw_want_die(client_val, &want); mk_lang_check_rereturn(err);
					if(want)
					{
						want_die = client_val;
					}
	...
	if(i == n)
	{
		...
		else
		{
			if(want_die)
			{
				err = mk_lib_iip_any_data_connections_rw_erase_value(&task->m_local.m_state.m_clients, &want_die); mk_lang_check_rereturn(err);
				*step_result = mk_lib_iip_cp_client_local_listener_task_result_e_did_something;
			}
			else
			{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_prrw_step_idle_do_stop(mk_lib_iip_cp_client_local_listener_task_pt const task, mk_lang_types_bool_t const allow_to_block, mk_lang_types_sint_t const tm, mk_lib_iip_cp_client_local_listener_task_result_pt const step_result) mk_lang_noexcept
{
	...
	{
		client_ptr = mk_lib_iip_any_data_connections_rw_at(&task->m_local.m_state.m_clients, i); mk_lang_assert(client_ptr); client_val = *client_ptr; mk_lang_assert(client_val);
		err = mk_lib_iip_cp_client_local_client_task_any_rw_request_close(client_val); mk_lang_check_rereturn(err);
	}
	*step_result = mk_lib_iip_cp_client_local_listener_task_result_e_did_something;
	return 0;
}
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_local_listener_task_any_prrw_allocate(mk_lib_iip_any_data_connection_ppt const connection) mk_lang_noexcept
{
	...
	self->m_local.m_settings.m_type.m_data.m_id = mk_lib_iip_any_func_connection_type_e_mk_lib_iip_cp_client_local_listener_task;
