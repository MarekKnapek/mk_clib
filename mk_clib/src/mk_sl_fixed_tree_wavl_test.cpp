...
mk_lang_nodiscard mk_lang_jumbo mk_lang_constexpr mk_lang_types_bool_t mk_sl_fixed_tree_wavl_test_fuzz(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	for(;;)
	{
		...
		for(j = 0; j != m; ++j)
		{
			err = mk_sl_fixed_tree_wavl_test_ro_find(&tree, mk_sl_fixed_tree_wavl_test_vec_ro_at(&vec, j), &node); mk_lang_test(err == 0); mk_lang_test(node); mk_lang_test(mk_sl_fixed_tree_wavl_test_ro_node_get_element(node, &pval) == 0 && *pval == *mk_sl_fixed_tree_wavl_test_vec_ro_at(&vec, j));
		}
		if(!(s >= 2)) break;
		n = ((mk_lang_types_sint_t)(((((mk_lang_types_uint_t)(mk_lang_types_uint_t)(d[0])) << 8)) | ((mk_lang_types_uint_t)(d[1])))); d += 2; s -= 2;
		for(i = 0; i != n; ++i)
		{
			...
			if(j != m)
			{
				err = mk_sl_fixed_tree_wavl_test_ro_find(&tree, &val, &node); mk_lang_test(err == 0); mk_lang_test(node); mk_lang_test(mk_sl_fixed_tree_wavl_test_ro_node_get_element(node, &pval) == 0 && *pval == val);
			...
			{
				err = mk_sl_fixed_tree_wavl_test_ro_find(&tree, &val, &node); mk_lang_test(err == 0); mk_lang_test(!node);
			}
		}
	}
