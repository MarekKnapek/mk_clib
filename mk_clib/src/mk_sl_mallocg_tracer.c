...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_sl_mallocg_tracer_db_rw_destruct(mk_sl_mallocg_tracer_db_pt const db) mk_lang_noexcept
{
	mk_lang_types_sint_t err;

	mk_lang_assert(db);
	mk_lang_assert(db->m_allocs.m_nodes == 0);
	mk_lang_assert(db->m_allocs.m_elements == 0);
	#if mk_lib_stack_trace_has
	mk_lang_assert(db->m_stacks.m_elements == 0);
	#endif

	#if mk_lib_stack_trace_has
	err = mk_sl_mallocg_tracer_stack_traces_rw_destruct(&db->m_stacks); mk_lang_check_rereturn(err);
	#endif
	err = mk_sl_mallocg_tracer_allocs_rw_destruct(&db->m_allocs); mk_lang_check_rereturn(err);
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_types_sint_t mk_sl_mallocg_tracer_deinit(mk_lang_types_void_t) mk_lang_noexcept
{
	...
	err = mk_sl_mallocg_tracer_db_rw_destruct(&g_mk_sl_mallocg_tracer_db); mk_lang_check_rereturn(err);
