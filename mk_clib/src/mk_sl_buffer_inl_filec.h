...
typedef mk_lang_bi_uchar_t const mk_lang_bi_uchar_ct; /* tdo move away */
typedef mk_lang_bi_uchar_t* mk_lang_bi_uchar_pt;
typedef mk_lang_bi_uchar_t const* mk_lang_bi_uchar_pct;
...
mk_lang_nodiscard mk_lang_jumbo mk_lang_void_pct mk_sl_buffer_inl_defd_buffer_ro_get_data_at(mk_sl_buffer_inl_defd_buffer_pct const buffer, mk_lang_size_t const idx) mk_lang_noexcept
{
	...
	return ((mk_lang_void_pct)(((mk_lang_bi_uchar_pct)(buffer->m_data)) + idx));
}
...
{
	return ((mk_lang_void_pt)(mk_sl_buffer_inl_defd_buffer_ro_get_data(buffer)));
}
...
{
	return ((mk_lang_void_pt)(mk_sl_buffer_inl_defd_buffer_ro_get_data_at(buffer, idx)));
}
...
mk_lang_jumbo void mk_sl_buffer_inl_defd_buffer_rw_resize(mk_sl_buffer_inl_defd_buffer_pt const buffer, mk_lang_exception_pt const ex, mk_lang_size_t const size) mk_lang_noexcept
{
	...
	if(buffer->m_size == 0)
	{
		mk_sl_buffer_inl_defd_mallocator_allocate(ex, size, &buffer->m_data);
	}
	else
	{
		mk_sl_buffer_inl_defd_mallocator_reallocate(ex, buffer->m_data, buffer->m_size, size, &buffer->m_data);
	}
