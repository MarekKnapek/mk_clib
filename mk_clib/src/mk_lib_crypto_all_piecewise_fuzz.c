...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_crypto_all_piecewise_fuzz_hashers(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	mk_lib_crypto_hash_stream_any2_append_u8s(&hasher_once, &buf[0], mk_lang_countof(buf) - rem);
	mk_lib_crypto_hash_stream_any2_finish(&hasher_once, &digest_once[0]);
	mk_lang_test(mk_sl_cui_uint8_memcmp_fn(&digest_piecewise[0], &digest_once[0], mk_lib_crypto_hash_stream_any2_digest_len(&hasher_piecewise)) == 0);
	return 0;
}
#include "mk_lang_warning_msvc_pop.h"
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_crypto_all_piecewise_fuzz_xofers(mk_lang_types_uchar_pct const data, mk_lang_types_usize_t const size) mk_lang_noexcept
{
	...
	mk_lib_crypto_xof_stream_any2_append_u8s(&xofer_once, &buf[0], mk_lang_countof(buf) - rem);
	...
	mk_lib_crypto_xof_stream_any2_squeeze_u8s(&xofer_once, &digest_once[0], mk_lang_countof(digest_once) - rem);
	mk_lang_test(mk_sl_cui_uint8_memcmp_fn(&digest_piecewise[0], &digest_once[0], mk_lang_countof(digest_once) - rem) == 0);
	return 0;
}
#include "mk_lang_warning_msvc_pop.h"
