...
mk_lang_constexpr static mk_lang_inline void mk_lib_crypto_hash_block_sha3_224_mix_block(mk_lib_crypto_hash_block_sha3_224_pt const sha3_224, mk_lib_crypto_hash_block_sha3_224_block_pct const block) mk_lang_noexcept
{
	int n mk_lang_constexpr_init;
	int i mk_lang_constexpr_init;
	mk_sl_cui_uint64_t ta mk_lang_constexpr_init;

	mk_lang_static_assert((mk_lib_crypto_hash_block_sha3_224_block_len % 8) == 0);

	mk_lang_assert(sha3_224);
	mk_lang_assert(block);

	n = mk_lib_crypto_hash_block_sha3_224_block_len / 8;
	for(i = 0; i != n; ++i)
	{
		mk_sl_uint_64_from_8_le(&ta, &block->m_uint8s[i * 8]);
		mk_sl_cui_uint64_xor2(&sha3_224->m_sha3.m_uint64s[i], &ta);
	}
}
...
mk_lang_constexpr mk_lang_jumbo void mk_lib_crypto_hash_block_sha3_224_append_blocks(mk_lib_crypto_hash_block_sha3_224_pt const sha3_224, mk_lib_crypto_hash_block_sha3_224_block_pct const pblocks, mk_lang_types_usize_t const nblocks) mk_lang_noexcept
{
	...
	mk_lang_assert(nblocks <= mk_lang_limits_usize_max / mk_lib_crypto_hash_block_sha3_224_block_len);
...
{
	#define digest_smol ((mk_lib_crypto_hash_block_sha3_224_digest_len % 8) != 0)
	...
	#if digest_smol
	mk_sl_cui_uint8_t digest_big[mk_lang_div_roundup(mk_lib_crypto_hash_block_sha3_224_digest_len, 8) * 8] mk_lang_constexpr_init;
	#endif

	mk_lang_assert(sha3_224);
	mk_lang_assert(block);
	mk_lang_assert(idx >= 0 && idx < mk_lib_crypto_hash_block_sha3_224_block_len);
	mk_lang_assert(digest);

	uch = 0x06;
	mk_sl_cui_uint8_from_bi_uchar(&block->m_uint8s[idx], &uch);
	rest = mk_lib_crypto_hash_block_sha3_224_block_len - idx - 1;
	for(i = 0; i != rest; ++i)
	{
	...
	mk_sl_cui_uint8_or2(&block->m_uint8s[mk_lib_crypto_hash_block_sha3_224_block_len - 1], &ta);
	...
	d = digest->m_uint8s;
	#endif
	rem = mk_lib_crypto_hash_block_sha3_224_digest_len;
	n = mk_lang_min(mk_lib_crypto_hash_block_sha3_224_block_len, rem);
	m = mk_lang_div_roundup(n, 8);
	for(i = 0; i != m; ++i)
	{
		mk_sl_uint_64_to_8_le(&sha3_224->m_sha3.m_uint64s[i], &d[i * 8]);
	}
	d += m * 8;
	rem -= m * 8;
	while(rem > 0)
