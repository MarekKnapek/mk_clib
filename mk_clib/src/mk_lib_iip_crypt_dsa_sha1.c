...
{
	mk_lib_iip_integer_dsa_sha1_pri_single_t dsa_sha1_q mk_lang_constexpr_init;
	mk_lang_types_bool_t ret mk_lang_constexpr_init;

	mk_lang_assert(pri);

	mk_lib_iip_integer_dsa_sha1_pri_single_load_q(&dsa_sha1_q);
	ret = !mk_lib_iip_integer_dsa_sha1_pri_single_is_zero(&pri->m_data.m_val) && mk_lib_iip_integer_dsa_sha1_pri_single_lt(&pri->m_data.m_val, &dsa_sha1_q);
	return ret;
}
...
mk_lang_jumbo mk_lang_types_void_t mk_lib_iip_crypt_dsa_sha1_key_pri_generate_random(mk_lib_iip_crypt_dsa_sha1_key_pri_pt const pri) mk_lang_noexcept
{
	...
	do
	{
		...
		mk_lib_iip_integer_dsa_sha1_pri_single_from_buis_uchar_le(&pri->m_data.m_val, &storage.m_data.m_uchars[0]);
