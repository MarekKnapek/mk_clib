...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_construct(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_client_shared_pt const shared, mk_lib_iip_cp_client_session_settings_pct const settings) mk_lang_noexcept
{
	...
	task->m_session.m_state.m_stop_acknowledged = mk_lang_false;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_on_msg_session_status_destroyed(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_message_pt const msg) mk_lang_noexcept
{
	mk_lang_assert(task);
	mk_lang_assert(msg);
	mk_lang_assert(task->m_session.m_state.m_has_id);
	mk_lang_assert(msg->m_header.m_type == mk_lib_iip_cp_message_message_type_id_e_session_status);
	mk_lang_assert(mk_lib_iip_cp_types_sessionid_eq(&msg->m_mix.m_data.m_session_status.m_session_id, &task->m_session.m_state.m_id));
	mk_lang_assert(msg->m_mix.m_data.m_session_status.m_status == mk_lib_iip_cp_message_session_status_status_id_e_destroyed);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_on_msg_session_status_created(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_message_pt const msg) mk_lang_noexcept
{
	mk_lang_assert(task);
	mk_lang_assert(msg);
	...
	mk_lang_assert(!task->m_session.m_state.m_has_id);
	mk_lang_assert(msg->m_header.m_type == mk_lib_iip_cp_message_message_type_id_e_session_status);
	mk_lang_assert(msg->m_mix.m_data.m_session_status.m_status == mk_lib_iip_cp_message_session_status_status_id_e_created);

	task->m_session.m_state.m_id = msg->m_mix.m_data.m_session_status.m_session_id;
	task->m_session.m_state.m_has_id = mk_lang_true;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_on_msg_session_status(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_message_pt const msg) mk_lang_noexcept
{
	...
	mk_lang_assert(task);
	mk_lang_assert(msg);
	mk_lang_assert(task->m_session.m_state.m_has_id || msg->m_mix.m_data.m_session_status.m_status == mk_lib_iip_cp_message_session_status_status_id_e_created);
	mk_lang_assert(mk_lib_iip_cp_types_sessionid_eq(&msg->m_mix.m_data.m_session_status.m_session_id, &task->m_session.m_state.m_id) || !task->m_session.m_state.m_has_id);

	msg_session_status = &msg->m_mix.m_data.m_session_status;
	switch(msg_session_status->m_status)
	{
		case mk_lib_iip_cp_message_session_status_status_id_e_destroyed: err = mk_lib_iip_cp_client_session_task_prrw_on_msg_session_status_destroyed(task, msg); mk_lang_check_rereturn(err); break;
		case mk_lib_iip_cp_message_session_status_status_id_e_created  : err = mk_lib_iip_cp_client_session_task_prrw_on_msg_session_status_created  (task, msg); mk_lang_check_rereturn(err); break;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_on_msg_request_lease_set(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_message_pt const msg) mk_lang_noexcept
{
	...
	msg_create_lease_set->m_leaseset.m_local_destination = task->m_session.m_settings.m_local_destination;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_on_incoming_streaming_packet(mk_lib_iip_cp_client_session_task_pt const task, mk_sl_cui_uint16_pct const src_port, mk_sl_cui_uint16_pct const dst_port, mk_sl_cui_uint8_pt const decompressed_packet_buf, mk_lang_types_sint_t const decompressed_packet_len, mk_lib_iip_net_streaming_packet_pt const packet) mk_lang_noexcept
{
	...
	err = mk_lib_iip_net_streaming_packet_ro_is_new_stream_initializator(packet, &is_first); mk_lang_check_rereturn(err);
	if(is_first)
	{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_step_make_msg_create_session(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_client_session_task_result_pt const step_result) mk_lang_noexcept
{
	...
	if(!waiting_for_master)
	{
		...
		msg_create_session->m_session_config.m_local_destination = task->m_session.m_settings.m_local_destination;
...
{
	mk_lib_iip_cp_client_socket_task_pt want_sent;
	mk_lang_types_usize_t n;
	mk_lang_types_usize_t i;
	mk_lang_types_usize_t idx;
	mk_lib_iip_cp_client_socket_task_ppt socket_ptr;
	...
	for(i = 0; i != n; ++i)
	{
		...
		stp_res = mk_lib_iip_cp_client_socket_task_result_e_dummy_end;
		err = mk_lib_iip_cp_client_socket_task_rw_step(socket_val, &stp_res); mk_lang_check_rereturn(err);
		switch(stp_res)
		{
			case mk_lib_iip_cp_client_socket_task_result_e_did_something: break2 = mk_lang_true; break;
			case mk_lib_iip_cp_client_socket_task_result_e_idling: /* nothing */ break;
			case mk_lib_iip_cp_client_socket_task_result_e_want_send: want_sent = socket_val; break;
			case mk_lib_iip_cp_client_socket_task_result_e_dummy_end: mk_lang_assert_false(); break;
			default: mk_lang_assert_false(); break;
		}
		if(break2)
		{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_step_idle_construct_destroy_session_msg(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_client_session_task_result_pt const step_result) mk_lang_noexcept
{
	...
	if(task->m_session.m_state.m_stop_requested && !task->m_session.m_state.m_stop_acknowledged)
	{
		if
		(
			(!task->m_session.m_state.m_has_msg_pending && !mk_lib_iip_cp_client_session_handle_is_zero(&task->m_session.m_settings.m_master_session)) ||
			(!task->m_session.m_state.m_has_msg_pending && mk_lib_iip_cp_client_session_handle_is_zero(&task->m_session.m_settings.m_master_session) && task->m_session.m_state.m_child_sessions == 0) ||
			(mk_lang_false)
		)
		{
			task->m_session.m_state.m_stop_acknowledged = mk_lang_true;
			...
			task->m_session.m_state.m_has_msg_pending = mk_lang_true;
			task->m_step = mk_lib_iip_cp_client_session_task_step_e_pickup_msg_destroy_session;
			stp_res = mk_lib_iip_cp_client_session_task_result_e_did_something;
		}
	}
	*step_result = stp_res;
	return 0;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_cp_client_session_task_prrw_connect_to(mk_lib_iip_cp_client_session_task_pt const task, mk_lib_iip_cp_client_types_socket_connect_settings_pct const settings, mk_lib_iip_cp_client_types_handle_socket_connect_pt const socket) mk_lang_noexcept
{
	...
	config.m_shared = task->m_session.m_state.m_shared;
	config.m_session_id = &task->m_session.m_state.m_id;
	config.m_is_listener = mk_lang_false;
	config.m_local_destination = &task->m_session.m_settings.m_local_destination;
