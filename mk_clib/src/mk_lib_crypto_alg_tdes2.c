...
mk_lang_constexpr static mk_lang_inline mk_lang_types_void_t mk_lib_crypto_alg_tdes2_schedule_crypt_all(mk_lib_crypto_alg_tdes2_schedule_pct const schedule, mk_lib_crypto_alg_tdes2_msg_pct const input, mk_lib_crypto_alg_tdes2_msg_pt const output, mk_lang_types_usize_t const nblocks) mk_lang_noexcept
{
	...
	mk_lang_assert(input);
	mk_lang_assert(output);
	mk_lang_assert(nblocks <= mk_lang_limits_usize_max / mk_lib_crypto_alg_tdes2_msg_len_m);

	n = nblocks;
	for(i = 0; i != n; ++i)
	{
		mk_lib_crypto_alg_tdes2_schedule_crypt_one(schedule, &input[i], &output[i]);
	}
}
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_alg_tdes2_schedule_encrypt(mk_lib_crypto_alg_tdes2_schedule_pct const schedule, mk_lib_crypto_alg_tdes2_msg_pct const input, mk_lib_crypto_alg_tdes2_msg_pt const output, mk_lang_types_usize_t const nblocks) mk_lang_noexcept
{
	...
	mk_lib_crypto_alg_tdes2_schedule_crypt_all(schedule, input, output, nblocks);
}
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_alg_tdes2_expand_enc(mk_lib_crypto_alg_tdes2_key_pct const key, mk_lib_crypto_alg_tdes2_schedule_pt const schedule) mk_lang_noexcept
{
	...
	mk_lib_crypto_alg_tdes2_expand(key, schedule);
}
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_alg_tdes2_expand_dec(mk_lib_crypto_alg_tdes2_key_pct const key, mk_lib_crypto_alg_tdes2_schedule_pt const schedule) mk_lang_noexcept
{
	...
	for(j = 0; j != 2; ++j)
	{
		for(i = 0; i != mk_lib_crypto_alg_tdes2_nr / 2; ++i)
		{
			ta = schedule->m_data.m_uint64s[j * mk_lib_crypto_alg_tdes2_nr + i];
			schedule->m_data.m_uint64s[j * mk_lib_crypto_alg_tdes2_nr + i] = schedule->m_data.m_uint64s[j * mk_lib_crypto_alg_tdes2_nr + (mk_lib_crypto_alg_tdes2_nr - 1) - i];
			schedule->m_data.m_uint64s[j * mk_lib_crypto_alg_tdes2_nr + (mk_lib_crypto_alg_tdes2_nr - 1) - i] = ta;
		}
	}
}
...
{
	mk_lib_crypto_alg_tdes2_schedule_t schedule mk_lang_constexpr_init;

	mk_lang_assert(key);
	mk_lang_assert(input);
	mk_lang_assert(output);

	mk_lib_crypto_alg_tdes2_expand_enc(key, &schedule);
	mk_lib_crypto_alg_tdes2_schedule_encrypt(&schedule, input, output, 1);
}
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_alg_tdes2_decrypt(mk_lib_crypto_alg_tdes2_key_pct const key, mk_lib_crypto_alg_tdes2_msg_pct const input, mk_lib_crypto_alg_tdes2_msg_pt const output) mk_lang_noexcept
{
	...
	mk_lib_crypto_alg_tdes2_expand_dec(key, &schedule);
	mk_lib_crypto_alg_tdes2_schedule_decrypt(&schedule, input, output, 1);
}
