...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_crypto_xof_stream_turbo_shake_256_fuzz_compute_ltc(mk_sl_cui_uint8_pct const data_buf, mk_lang_types_uint_t const data_len, mk_lang_static_param(mk_sl_cui_uint8_t, digest, mk_lib_crypto_xof_stream_turbo_shake_256_fuzz_bytes_count)) mk_lang_noexcept
{
	...
	st = turbo_shake_init(&xofer_ltc, 256); mk_lang_check_return(st == CRYPT_OK);
	st = turbo_shake_process(&xofer_ltc, ((mk_lang_types_uchar_pct)(data_buf)), data_len); mk_lang_check_return(st == CRYPT_OK);
	st = turbo_shake_done(&xofer_ltc, ((mk_lang_types_uchar_pt)(digest)), mk_lib_crypto_xof_stream_turbo_shake_256_fuzz_bytes_count); mk_lang_check_return(st == CRYPT_OK);
	return 0;
}
#endif
