...
{
	mk_lang_alignas(mk_lib_crypto_hash_block_sha0_block_len_v) mk_lib_crypto_hash_block_sha0_block2_data_t m_data;
};
...
mk_lang_constexpr_static_inline mk_sl_cui_uint32_t const mk_lib_crypto_hash_block_sha0_k_init[5] =
{
...
mk_lang_constexpr_static_inline mk_sl_cui_uint32_t const mk_lib_crypto_hash_block_sha0_k_table[4] =
{
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_hash_block_sha0_init(mk_lib_crypto_hash_block_sha0_pt const sha0) mk_lang_noexcept
{
	...
	mk_lib_crypto_bitops_bulk_uint32_5_memcpy(&sha0->m_state[0], &mk_lib_crypto_hash_block_sha0_k_init[0]);
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_hash_block_sha0_append_blocks(mk_lib_crypto_hash_block_sha0_pt const sha0, mk_lib_crypto_hash_block_sha0_block_pct const pblocks, mk_lang_types_usize_t const nblocks) mk_lang_noexcept
{
	...
	if(nblocks != 0)
	{
		...
		mk_lib_crypto_bitops_bulk_uint32_5_memcpy(&old_state[0], &sha0->m_state[0]);
		for(iblock = 0; iblock != nblocks; ++iblock)
		{
			...
			for(ir = 0; ir != 80; ++ir)
			{
				...
				else if(ir >= 16)
				{
					...
					mk_sl_cui_uint32_xor3(&ta, &tb, &w.m_data.m_u32s[(ir - 0) % 16]);
				}
				else
				{
				...
				mk_lib_crypto_hash_block_sha0_f(ir, b, c, d, &tb);
				mk_sl_cui_uint32_add2_wrap_cid_cod(&ta, &tb);
				mk_sl_cui_uint32_add3_wrap_cid_cod(e, &w.m_data.m_u32s[(ir - 0) % 16], &tb);
				mk_sl_cui_uint32_add2_wrap_cid_cod(&tb, &mk_lib_crypto_hash_block_sha0_k_table[ir / 20]);
		...
		mk_lib_crypto_bitops_bulk_uint32_5_memcpy(&sha0->m_state[0], &old_state[0]);
	}
}
#include "mk_lang_warning_msvc_pop.h"
...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_hash_block_sha0_finish(mk_lib_crypto_hash_block_sha0_pt const sha0, mk_lib_crypto_hash_block_sha0_block_pt const block, mk_lang_types_sint_t const idx, mk_lib_crypto_hash_block_sha0_digest_pt const digest) mk_lang_noexcept
{
	...
	else
	{
		...
		mk_sl_cui_uint8_memclr_fn(&block->m_data.m_uint8s[0], ((mk_lang_types_sint_t)(mk_lib_crypto_hash_block_sha0_block_len_v)) - ((mk_lang_types_sint_t)(mk_sl_cui_uint64_size_bytes_v)));
	}

	mk_sl_uint_convert_64_8_be_to_sml(&ta, &block->m_data.m_uint8s[((mk_lang_types_sint_t)(mk_lib_crypto_hash_block_sha0_block_len_v)) - ((mk_lang_types_sint_t)(mk_sl_cui_uint64_size_bytes_v))]);
	mk_lib_crypto_hash_block_sha0_append_blocks(sha0, block, 1);
	for(i = 0; i != mk_lang_countof(sha0->m_state); ++i)
	{
		mk_sl_uint_convert_32_8_be_to_sml(&sha0->m_state[i], &digest->m_data.m_uint8s[i * mk_sl_cui_uint32_size_bytes_v]);
	}
}
#include "mk_lang_warning_msvc_pop.h"
