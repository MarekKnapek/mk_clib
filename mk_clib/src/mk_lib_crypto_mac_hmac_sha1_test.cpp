...
mk_lang_extern_c void mk_lib_crypto_mac_hmac_sha1_test(void) mk_lang_noexcept
{
	...
	mk_lang_constexpr_static auto const s_key_computed_1 = mk_lib_crypto_mac_hmac_sha1_test_compute_from_str_lit(msg_1, key_1);
	mk_lang_constexpr_static auto const s_key_computed_2 = mk_lib_crypto_mac_hmac_sha1_test_compute_from_str_lit(msg_2, key_2);
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lib_crypto_mac_hmac_sha1_init(&hmac, &key[0], s_key_lens[i]);
		mk_lib_crypto_mac_hmac_sha1_append(&hmac, &msg[0], s_msg_lens[i]);
		mk_lib_crypto_mac_hmac_sha1_finish(&hmac, &digest);
		m = s_mac_lens[i];
		for(j = 0; j != m; ++j)
		{
