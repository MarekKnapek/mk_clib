...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_hash_stream_blake2s_256_test_compute_from_str_lit(char const(&str_lit)[str_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_cpp_constexpr_array_t<mk_lang_types_uchar_t, mk_lib_crypto_hash_block_blake2s_256_digest_len> ret mk_lang_constexpr_init;

	mk_lang_static_assert(str_lit_len >= 1);

	mk_lib_crypto_hash_stream_blake2s_256_init(&blake2s_256);
	for(i = 0; i  != s_len; ++i){ uch = ((mk_lang_types_uchar_t)(str_lit[i])); mk_lib_crypto_hash_stream_blake2s_256_append(&blake2s_256, &uch, 1); }
	mk_lib_crypto_hash_stream_blake2s_256_finish(&blake2s_256, &digest);
	for(i = 0; i != mk_lib_crypto_hash_block_blake2s_256_digest_len; ++i){ mk_sl_cui_uint8_to_bi_uchar(&digest.m_uint8s[i], &ret[i]); }
	return ret;
}
