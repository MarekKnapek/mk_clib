#undef mk_sl_flt_defd_bui_tn
#undef mk_sl_flt_defd_bui_t
#undef mk_sl_flt_defd_bui_sizeof
#undef mk_sl_flt_defd_bui_bits
#undef mk_sl_flt_defd_has_bits_a
#undef mk_sl_flt_defd_has_bits_b

#undef mk_sl_flt_defd_cui
#undef mk_sl_flt_defd_cui_t
#undef mk_sl_flt_defd_cui_set_zero
#undef mk_sl_flt_defd_cui_to_bi_sint
#undef mk_sl_flt_defd_cui_from_bi_sint
#undef mk_sl_flt_defd_cui_to_buis_uchar_le
#undef mk_sl_flt_defd_cui_from_buis_uchar_le
#undef mk_sl_flt_defd_cui_set_bit
#undef mk_sl_flt_defd_cui_set_mask
#undef mk_sl_flt_defd_cui_is_zero
#undef mk_sl_flt_defd_cui_or2
#undef mk_sl_flt_defd_cui_and3
#undef mk_sl_flt_defd_cui_and2
#undef mk_sl_flt_defd_cui_shr3
#undef mk_sl_flt_defd_cui_shl2

#undef mk_sl_flt_defd_cuiba
#undef mk_sl_flt_defd_cuiba_t
#undef mk_sl_flt_defd_cuiba_from_bi_sint
#undef mk_sl_flt_defd_cuiba_count_leading_zeros
#undef mk_sl_flt_defd_cuiba_is_zero
#undef mk_sl_flt_defd_cuiba_shl2
#undef mk_sl_flt_defd_cuiba_shr2
#undef mk_sl_flt_defd_cuiba_add3_wrap_cid_coe
#undef mk_sl_flt_defd_cuiba_add3_wrap_cid_coe_smol
#undef mk_sl_flt_defd_cuiba_mul4_wrap_wi_smol
#undef mk_sl_flt_defd_cuiba_to_str_dec_n

#undef mk_sl_flt_defd_cuibb
#undef mk_sl_flt_defd_cuibb_t
#undef mk_sl_flt_defd_cuibb_is_zero
#undef mk_sl_flt_defd_cuibb_shl2
#undef mk_sl_flt_defd_cuibb_mul4_wrap_wi_smol

#undef mk_sl_flt_defd_convert_to_biga
#undef mk_sl_flt_defd_convert_to_num
#undef mk_sl_flt_defd_convert_to_bigb
