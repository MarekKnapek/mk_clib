...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_mode_cfb_8_aes_192_test_encrypt_from_str_lit(char const(&iv_lit)[iv_lit_len], char const(&key_lit)[key_lit_len], char const(&msg_lit)[msg_lit_len]) mk_lang_noexcept
{
	...
	{
		for(i = 0; i != mk_lib_crypto_mode_cfb_8_aes_192_msg_len_v; ++i){ msg.m_data.m_uint8s[i] = msg_bytes[j * mk_lib_crypto_mode_cfb_8_aes_192_msg_len_v + i]; }
		mk_lib_crypto_mode_cfb_8_aes_192_encrypt(&mode, &key, &msg, &msg);
		for(i = 0; i != mk_lib_crypto_mode_cfb_8_aes_192_msg_len_v; ++i){ out_bytes[j * mk_lib_crypto_mode_cfb_8_aes_192_msg_len_v + i] = msg.m_data.m_uint8s[i]; }
	}
	return out_bytes;
}
