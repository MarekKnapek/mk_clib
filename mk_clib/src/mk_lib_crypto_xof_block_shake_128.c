...
mk_lang_constexpr static mk_lang_inline void mk_lib_crypto_xof_block_shake_128_mix_block(mk_lib_crypto_xof_block_shake_128_pt const shake_128, mk_lib_crypto_xof_block_shake_128_block_pct const block) mk_lang_noexcept
{
...
mk_lang_constexpr mk_lang_jumbo void mk_lib_crypto_xof_block_shake_128_finish(mk_lib_crypto_xof_block_shake_128_pt const shake_128, mk_lib_crypto_xof_block_shake_128_block_pt const block, int const idx, mk_lang_types_usize_t const digest_len, mk_lib_crypto_xof_block_shake_128_digest_pt const digest) mk_lang_noexcept
{
	...
	mk_sl_cui_uint8_or2(&block->m_uint8s[mk_lib_crypto_xof_block_shake_128_block_len - 1], &ta);
	mk_lib_crypto_xof_block_shake_128_mix_block(shake_128, block);
	rem = digest_len;
	out = 0;
	n = rem / mk_lib_crypto_xof_block_shake_128_block_len;
	for(j = 0; j != n; ++j)
	{
		mk_lib_crypto_hash_block_sha3_base_f(&shake_128->m_sha3);
		m = mk_lib_crypto_xof_block_shake_128_block_len / 8;
		for(i = 0; i != m; ++i, out += 8)
		{
			mk_sl_uint_64_to_8_le(&shake_128->m_sha3.m_uint64s[i], &get_digest(digest, out));
		}
	}
	rem -= n * mk_lib_crypto_xof_block_shake_128_block_len;
	...
	if(rem != 0)
	{
		...
		mk_sl_uint_64_to_8_le(&shake_128->m_sha3.m_uint64s[i], big);
		m = ((int)(rem));
		for(i = 0; i != m; ++i, ++out)
		{
