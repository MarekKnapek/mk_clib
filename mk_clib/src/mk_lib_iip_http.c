...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_bool_t mk_lib_iip_http_prrw_is_space(mk_lib_iip_http_pt const http, mk_lang_types_sint_t const pos) mk_lang_noexcept
{
	...
	mk_sl_cui_uint8_to_bi_pchar(mk_lib_iip_http_buffer_rw_get_at(&http->m_buffer, ((mk_lang_types_usize_t)(pos))), &tpc);
	is = tpc == ' ';
	return is;
}

mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_is_string(mk_lib_iip_http_pt const http, mk_lib_iip_http_strings_id_t const str_id, mk_lang_types_bool_pt const is) mk_lang_noexcept
{
	mk_lang_types_sint_t err;
	...
	err = mk_lib_iip_http_buffer_rw_consolidate(&http->m_buffer); mk_lang_check_rereturn(err);
	dat_buf = mk_lib_iip_http_buffer_rw_get_data_a(&http->m_buffer);
	dat_sise = mk_lib_iip_http_buffer_rw_get_sise_a(&http->m_buffer);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_find_space(mk_lib_iip_http_pt const http, mk_lang_types_sint_pt const pos) mk_lang_noexcept
{
	mk_lang_types_sint_t err;
	mk_sl_cui_uint8_pct dat_buf;
	mk_lang_types_sint_t dat_sise;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_find_colon_space(mk_lib_iip_http_pt const http, mk_lang_types_sint_pt const pos) mk_lang_noexcept
{
	mk_lang_types_sint_t err;
	mk_sl_cui_uint8_pct dat_buf;
	mk_lang_types_sint_t dat_sise;
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_find_crlf(mk_lib_iip_http_pt const http, mk_lang_types_sint_pt const pos) mk_lang_noexcept
{
	mk_lang_types_sint_t err;
	mk_sl_cui_uint8_pct dat_buf;
	mk_lang_types_sint_t dat_sise;
...
#include "mk_lang_warning_clang_push_conditional_uninitialized.h"
	...
	dat_sise = mk_lib_iip_http_buffer_rw_get_sise(&http->m_buffer);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_on_incoming_data_uri_work(mk_lib_iip_http_pt const http, mk_sl_cui_uint8_pct const data_buf, mk_lang_types_sint_t const data_len, mk_lib_iip_http_parse_error_code_pt const error_code, mk_lang_types_sint_pt const consumed) mk_lang_noexcept
{
	...
	*consumed = data_len;
	err = mk_lib_iip_http_prrw_find_space(http, &pos); mk_lang_check_rereturn(err);
	if(pos != -1)
	{
		dat_buf = mk_lib_iip_http_buffer_rw_get_data_a(&http->m_buffer);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_on_incoming_data_version_work(mk_lib_iip_http_pt const http, mk_sl_cui_uint8_pct const data_buf, mk_lang_types_sint_t const data_len, mk_lib_iip_http_parse_error_code_pt const error_code, mk_lang_types_sint_pt const consumed) mk_lang_noexcept
{
	...
	err = mk_lib_iip_http_buffer_rw_push_back_copy_many(&http->m_buffer, data_buf, ((mk_lang_types_usize_t)(rem))); mk_lang_check_rereturn(err);
	*consumed = rem;
	err = mk_lib_iip_http_prrw_is_string(http, mk_lib_iip_http_strings_id_e_version_1_1, &is); mk_lang_check_rereturn(err);
	if(is)
	{
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_on_incoming_data_headers_work(mk_lib_iip_http_pt const http, mk_sl_cui_uint8_pct const data_buf, mk_lang_types_sint_t const data_len, mk_lib_iip_http_parse_error_code_pt const error_code, mk_lang_types_sint_pt const consumed) mk_lang_noexcept
{
	...
	*consumed = data_len;
	for(;;)
	{
		err = mk_lib_iip_http_prrw_find_crlf(http, &pos_crlf); mk_lang_check_rereturn(err);
		if(pos_crlf == 0)
		{
		...
		{
			err = mk_lib_iip_http_prrw_find_colon_space(http, &pos_sep); mk_lang_check_rereturn(err);
...
mk_lang_nodiscard static mk_lang_inline mk_lang_types_sint_t mk_lib_iip_http_prrw_on_incoming_data(mk_lib_iip_http_pt const http, mk_sl_cui_uint8_pct const data_buf, mk_lang_types_sint_t const data_len, mk_lib_iip_http_parse_error_code_pt const error_code, mk_lang_types_sint_pt const consumed) mk_lang_noexcept
{
	...
	if(mk_lib_iip_http_prrw_on_incoming_data_method_has (http) && !mk_lib_iip_http_prrw_on_incoming_data_uri_has    (http)){ err = mk_lib_iip_http_prrw_on_incoming_data_uri_work    (http, ptr, rem, error_code, &tlen); mk_lang_check_rereturn(err); if(*error_code != mk_lib_iip_http_parse_error_code_e_ok){ return 0;} mk_lang_assert(tlen >= 0); mk_lang_assert(tlen <= rem); ptr += tlen; rem -= tlen; }
	if(mk_lib_iip_http_prrw_on_incoming_data_uri_has    (http) && !mk_lib_iip_http_prrw_on_incoming_data_version_has(http)){ err = mk_lib_iip_http_prrw_on_incoming_data_version_work(http, ptr, rem, error_code, &tlen); mk_lang_check_rereturn(err); if(*error_code != mk_lib_iip_http_parse_error_code_e_ok){ return 0;} mk_lang_assert(tlen >= 0); mk_lang_assert(tlen <= rem); ptr += tlen; rem -= tlen; }
	if(mk_lib_iip_http_prrw_on_incoming_data_version_has(http) && !mk_lib_iip_http_prrw_on_incoming_data_headers_has(http)){ err = mk_lib_iip_http_prrw_on_incoming_data_headers_work(http, ptr, rem, error_code, &tlen); mk_lang_check_rereturn(err); if(*error_code != mk_lib_iip_http_parse_error_code_e_ok){ return 0;} mk_lang_assert(tlen >= 0); mk_lang_assert(tlen <= rem); ptr += tlen; rem -= tlen; }
	tlen = data_len - rem;
	*consumed = tlen;
	return 0;
}
