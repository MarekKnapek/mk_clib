...
mk_lang_constexpr mk_lang_jumbo mk_lang_types_void_t mk_lib_crypto_xof_block_shake_256_squeeze(mk_lib_crypto_xof_block_shake_256_pt const shake_256, mk_lang_types_usize_t const len, mk_sl_cui_uint8_pt const digest) mk_lang_noexcept
{
	...
	#define words_per_block ((mk_lang_types_sint_t)(mk_lib_crypto_xof_block_shake_256_block_len / bytes_per_word))
	...
	mk_lang_assert(shake_256->m_idx >= 0 && shake_256->m_idx < mk_lib_crypto_xof_block_shake_256_block_len);

	rem = len;
	out = digest;
	if(shake_256->m_idx != 0)
	{
		nfbytes = bytes_per_word - (shake_256->m_idx - ((shake_256->m_idx / bytes_per_word) * bytes_per_word));
		nfbytes = ((mk_lang_types_sint_t)(mk_lang_min(((mk_lang_types_usize_t)(nfbytes)), rem)));
		nfbytes = nfbytes % bytes_per_word;
		if(nfbytes != 0)
		{
			ifword = shake_256->m_idx / bytes_per_word;
			mk_sl_uint_64_to_8_le(&shake_256->m_sha3.m_uint64s[ifword], &first_word[0]);
			for(ifbyte = 0; ifbyte != nfbytes; ++ifbyte)
			{
				out[ifbyte] = first_word[shake_256->m_idx % bytes_per_word + ifbyte];
			}
			rem = rem - nfbytes;
			out = out + nfbytes;
			shake_256->m_idx = (shake_256->m_idx + nfbytes) % mk_lib_crypto_xof_block_shake_256_block_len;
		}
		nfwords = (mk_lib_crypto_xof_block_shake_256_block_len - shake_256->m_idx) / bytes_per_word;
		nfwords = ((mk_lang_types_sint_t)(mk_lang_min(((mk_lang_types_usize_t)(nfwords)), rem / bytes_per_word)));
		nfwords = nfwords % words_per_block;
		if(nfwords != 0 && shake_256->m_idx == 0)
		{
			mk_lib_crypto_hash_block_sha3_base_f(&shake_256->m_sha3);
		}
		for(ifword = 0; ifword != nfwords; ++ifword, out += bytes_per_word)
		{
			mk_sl_uint_64_to_8_le(&shake_256->m_sha3.m_uint64s[shake_256->m_idx / bytes_per_word + ifword], out);
		}
		rem = rem - nfwords * bytes_per_word;
		shake_256->m_idx = (shake_256->m_idx + nfwords * bytes_per_word) % mk_lib_crypto_xof_block_shake_256_block_len;
	}
	nblocks = rem / mk_lib_crypto_xof_block_shake_256_block_len;
	for(iblock = 0; iblock != nblocks; ++iblock)
	{
		mk_lang_assert(shake_256->m_idx == 0);
		mk_lib_crypto_hash_block_sha3_base_f(&shake_256->m_sha3);
	...
	if(rem != 0)
	{
		...
		for(iword = 0; iword != nwords; ++iword, out += bytes_per_word)
		{
			mk_sl_uint_64_to_8_le(&shake_256->m_sha3.m_uint64s[shake_256->m_idx / bytes_per_word + iword], out);
		}
		...
		if(rem != 0)
		{
			...
			{
				out[ibyte] = last_word[shake_256->m_idx % bytes_per_word + ibyte];
			}
			shake_256->m_idx = (shake_256->m_idx + rem) % mk_lib_crypto_xof_block_shake_256_block_len;
		}
	}
