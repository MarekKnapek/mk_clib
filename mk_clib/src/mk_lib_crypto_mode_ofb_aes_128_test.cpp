...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_mode_ofb_aes_128_test_encrypt_from_str_lit(char const(&iv_lit)[iv_lit_len], char const(&key_lit)[key_lit_len], char const(&msg_lit)[msg_lit_len]) mk_lang_noexcept
{
	...
	for(i = 0; i != mk_lib_crypto_mode_ofb_aes_128_msg_len_v; ++i){ iv.m_data.m_uint8s[i] = iv_bytes[i]; }
	mk_lib_crypto_mode_ofb_aes_128_init(&mode, &iv);
	for(i = 0; i != mk_lib_crypto_mode_ofb_aes_128_key_len_v; ++i){ key.m_data.m_uint8s[i] = key_bytes[i]; }
	for(j = 0; j != ((msg_lit_len - 1) / 2) / mk_lib_crypto_mode_ofb_aes_128_msg_len_v; ++j)
	{
		for(i = 0; i != mk_lib_crypto_mode_ofb_aes_128_msg_len_v; ++i){ msg.m_data.m_uint8s[i] = msg_bytes[j * mk_lib_crypto_mode_ofb_aes_128_msg_len_v + i]; }
		mk_lib_crypto_mode_ofb_aes_128_encrypt(&mode, &key, &msg, &msg);
		for(i = 0; i != mk_lib_crypto_mode_ofb_aes_128_msg_len_v; ++i){ out_bytes[j * mk_lib_crypto_mode_ofb_aes_128_msg_len_v + i] = msg.m_data.m_uint8s[i]; }
	}
	return out_bytes;
}
