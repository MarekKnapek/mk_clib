...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_hash_stream_blake2s_160_test_compute_from_str_lit(char const(&str_lit)[str_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_crypto_hash_block_blake2s_160_digest_t digest mk_lang_constexpr_init;
	mk_lib_cpp_constexpr_array_t<mk_lang_types_uchar_t, mk_lib_crypto_hash_block_blake2s_160_digest_len> ret mk_lang_constexpr_init;

	mk_lang_static_assert(str_lit_len >= 1);

	mk_lib_crypto_hash_stream_blake2s_160_init(& blake2s_160);
	for(i = 0; i  != s_len; ++i){ uch = ((mk_lang_types_uchar_t)(str_lit[i])); mk_lib_crypto_hash_stream_blake2s_160_append(& blake2s_160, &uch, 1); }
	mk_lib_crypto_hash_stream_blake2s_160_finish(& blake2s_160, &digest);
	for(i = 0; i != mk_lib_crypto_hash_block_blake2s_160_digest_len; ++i){ mk_sl_cui_uint8_to_bi_uchar(&digest.m_uint8s[i], &ret[i]); }
	return ret;
}
...
mk_lang_extern_c void mk_lib_crypto_hash_stream_blake2s_160_test(void) mk_lang_noexcept
{
	#define message_1 ""
	#define message_2 "a"
	...
	#define digest_8 "4046daae0c27ba592b0e7e6de8db8893a3cec8b2"
	...
	int n;
	int i;
	mk_lib_crypto_hash_stream_blake2s_160_t  blake2s_160;
	...
	for(i = 0; i != n; ++i)
	{
		...
		mk_lib_crypto_hash_stream_blake2s_160_init(& blake2s_160);
		mk_lib_crypto_hash_stream_blake2s_160_append(& blake2s_160, ((mk_lang_types_uchar_pct)(s_messages[i])), s_message_lens[i]);
		mk_lib_crypto_hash_stream_blake2s_160_finish(& blake2s_160, &digest);
		for(j = 0; j != mk_lib_crypto_hash_block_blake2s_160_digest_len; ++j)
		{
