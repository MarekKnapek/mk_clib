...
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_mode_cfb_8_aes_128_test_encrypt_from_str_lit(char const(&iv_lit)[iv_lit_len], char const(&key_lit)[key_lit_len], char const(&msg_lit)[msg_lit_len]) mk_lang_noexcept
{
	...
	mk_lib_cpp_constexpr_array_t<mk_lang_types_uchar_t, mk_lib_crypto_mode_cfb_8_aes_128_key_len_v> key_bytes mk_lang_constexpr_init;
	...
	for(i = 0; i != mk_lib_crypto_mode_cfb_8_aes_128_key_len_v; ++i){ key.m_data.m_uchars[i] = key_bytes[i]; }
	for(j = 0; j != ((msg_lit_len - 1) / 2) / mk_lib_crypto_mode_cfb_8_aes_128_msg_len_v; ++j)
	{
		for(i = 0; i != mk_lib_crypto_mode_cfb_8_aes_128_msg_len_v; ++i){ msg.m_data.m_uchars[i] = msg_bytes[j * mk_lib_crypto_mode_cfb_8_aes_128_msg_len_v + i]; }
		mk_lib_crypto_mode_cfb_8_aes_128_encrypt(&mode, &key, &msg, &msg);
		for(i = 0; i != mk_lib_crypto_mode_cfb_8_aes_128_msg_len_v; ++i){ out_bytes[j * mk_lib_crypto_mode_cfb_8_aes_128_msg_len_v + i] = msg.m_data.m_uchars[i]; }
	}
	return out_bytes;
}

template<mk_lang_types_usize_t iv_lit_len, mk_lang_types_usize_t key_lit_len, mk_lang_types_usize_t msg_bytes_len>
mk_lang_nodiscard mk_lang_constexpr mk_lang_jumbo auto mk_lib_crypto_mode_cfb_8_aes_128_test_decrypt_from_str_lit(char const(&iv_lit)[iv_lit_len], char const(&key_lit)[key_lit_len], mk_lib_cpp_constexpr_array_t<mk_lang_types_uchar_t, msg_bytes_len> const& msg_bytes) mk_lang_noexcept
{
	mk_lib_cpp_constexpr_array_t<mk_lang_types_uchar_t, mk_lib_crypto_alg_aes_128_msg_len_v> iv_bytes mk_lang_constexpr_init;
	mk_lib_cpp_constexpr_array_t<mk_lang_types_uchar_t, mk_lib_crypto_mode_cfb_8_aes_128_key_len_v> key_bytes mk_lang_constexpr_init;
	mk_lib_crypto_alg_aes_128_msg_t iv mk_lang_constexpr_init;
